...
### ☑ Session 99 — live_momentum scalar fair-value proxy (May 11, Outcome A instrumentation-only ship)

**Trigger.** CLAUDE.md "Data Collection Backlog" Priority 3: *"live_momentum still lacks a reliable scalar predicted probability. Calibration and sizing are weaker because the strategy logs price-action decisions but not a comparable 'our fair' value at entry."* This is load-bearing substrate for the Future Direction → Closed-Loop Strategy Lifecycle prerequisite of confidence-weighted sizing. Without a scalar predicted probability per live_momentum entry, every accepted trade is fixed-size and calibration/sizing improvements are blocked. Session 96 already shipped `_build_live_momentum_decision_context()` ([bot/live_watcher.py:128-243](bot/live_watcher.py:128)) which extracts every input the proxy needs; Session 99 layers a scalar on top. Sizing changes are explicitly off-limits per backlog rule — first measure calibration; size later.

**Phase 0 — verify the premise before scoping.**

- **0.1 math identity (key finding).** Reading [bot/live_watcher.py:189](bot/live_watcher.py:189) directly: `wp_edge = float(game_ctx.win_probability) - (float(ask_num) / 100.0)`. Reading [bot/live_watcher.py:212](bot/live_watcher.py:212): `leader_price = _compact_number(yes_ask, ndigits=1)`. Therefore `estimated_win_prob = wp_edge + leader_price/100` **reconstructs `game_ctx.win_probability` exactly** when both inputs are present. The "fair-value proxy" the user wanted is actually a 1:1 lift of an existing scalar that wasn't being persisted. No new model math needed; v0 is "persist the scalar that already exists."
- **0.2 field coverage (parallel Explore agent).** All required inputs (`wp_edge`, `leader_price`) are 100% present in the 13 context-available live_momentum decisions of the last 500. Sample is thin (≈4/day) but the proxy is forward-only — 14-day observation window starts at restart.
- **0.3 calibration_report.py extension surface (parallel Explore agent).** Current report at [tools/calibration_report.py](tools/calibration_report.py) reads `predictions.jsonl` + groups by `opp_type`; live_momentum has been excluded as `predicted_fair_cents=None` per the documented Session-7 gap. Light-refactor (~80 LOC) — add a separate section reading paper_trades.json. Going with paper_trades.json read (Option B) over predictions.jsonl write (Option A from the design agent) to preserve predictions.jsonl as vig_stack-only and honor the spec's "separate this proxy from vig_stack fair-value calibration" wording.
- **0.4 wire-up point (key finding).** Single correct injection at [bot/live_watcher.py:1422-1453](bot/live_watcher.py:1422) — where `primary_context` and `opponent_context` are built once per tick using the real `self._game_ctx`. Both contexts are reused by 8+ downstream reject sites + the accept site at line 1889. Injecting here means every consumer inherits the new keys via the existing `_annotate_live_momentum_context()` helper (which preserves dict keys). Do NOT inject at the 6 scan-time call sites (3320/3382/3544/3587/3633/3659) — those pass `game_ctx=None` so the proxy correctly returns `None`; adding redundant calls would clutter scan paths. Do NOT re-call the proxy in the conviction branch (~line 1788 where `wp_edge` gets overwritten) — the primary_context value built at line 1422 is canonical.

**Outcome A (ship).** Phase 0 evidence supports the simplest possible v0 proxy: a direct lift of `game_context.win_probability`, persisted as `estimated_win_prob` on live_momentum decisions and accepted trades. Instrumentation-only.

**Implementation.**
- **New module `bot/live_momentum_proxy.py`** (~60 LOC, stdlib only). `estimate_live_momentum_win_prob(context: dict | None) -> dict` returns `{estimated_win_prob, model_source, confidence_components}`. v0 math: `clamp(wp_edge + leader_price/100, 0.05, 0.95)`. `model_source = "game_context_win_probability_v1"` — explicit v1 so future variants live at `_v2`. `confidence_components` carries `{wp_edge, market_implied, pre_clamp_raw, clamped}` for calibration diagnostics. Returns None gracefully when context is missing, when either required input (wp_edge, leader_price) is absent, or when an input is non-numeric.
- **Wire into per-tick context builds at [bot/live_watcher.py:1422-1453](bot/live_watcher.py:1422).** Right after each `primary_context = _build_live_momentum_decision_context(...)` and `opponent_context = _build_live_momentum_decision_context(...)`, call the proxy and merge `estimated_win_prob` / `model_source` / `confidence_components` into the context dict. Every downstream `_annotate_live_momentum_context()` call (8 reject sites + accept site) inherits the new keys via the existing dict-copy semantics — no changes to those sites required.
- **Surface on opp dict at [bot/live_watcher.py:2115-2142](bot/live_watcher.py:2115)** (in `_auto_bet_momentum`). Added `paper_estimated_win_prob`, `paper_model_source`, `paper_confidence_components` keys reading from `decision_context` (which IS the proxy-enriched context flowing in). Session 50 forward-only `paper_*` pattern mirrored exactly.
- **Persist on paper trade record at [bot/executor.py:1188-1207](bot/executor.py:1188).** Three conditional adds just before `paper_trades.append(record)`, mirroring the Session 50 dqs/sport/confidence pattern. Vig_stack opp dicts don't carry the `paper_*` keys, so vig_stack records stay byte-identical to pre-Session-99 — preserves the spillover-prevention invariant.
- **Extend [tools/calibration_report.py](tools/calibration_report.py)** with `report_live_momentum_calibration(days)` reading `bot/state/paper_trades.json` filtered to `type=="live_momentum"` AND `status in ("won", "lost")` AND `estimated_win_prob is not None`. Buckets by probability in 0.1-wide bins from 0.5 to 1.0. Computes Brier on (predicted_prob, 1 if won else 0). Renders as a separate `## live_momentum proxy calibration (n=N)` section after the existing vig_stack output. `__main__` wired to also `print()` the new section.

**Tests.**
- **`tests/test_live_momentum_proxy.py` (new, 9 tests).** Unit coverage: expected scalar for known inputs, floor clamp, ceiling clamp, None on missing wp_edge / missing leader_price / non-dict context / non-numeric input, model_source string stability, math identity reconstructs game_ctx.win_probability across 3 sample points.
- **`tests/test_bot_executor.py::TestSession99PaperTradeProxyFields` (new, 4 tests).** Mirror Session 50 pattern: round-trip persists 3 fields on live_momentum record, spillover-prevention on vig_stack (no fields appear), partial fields handled, non-numeric estimated_win_prob silently dropped.
- **`tests/test_live_watcher.py` (3 new tests).** `_annotate_live_momentum_context` preserves the 3 proxy keys (locks pass-through against future refactor), proxy fields land in `decisions.jsonl.extra` on a reject path, proxy returns None when game_ctx absent (validates the scan-time call-site behavior).
- **`tests/test_calibration_report.py` (new, 8 tests).** Missing paper_trades file renders caveat, empty paper_trades renders 'ships forward-only' caveat, unreadable JSON renders 'unreadable' caveat, non-list schema rejects, only-unresolved trades render n=0, pre-Session-99 records (no field) silently dropped, populated case renders full Brier+bucket calibration, vig_stack records excluded from live_momentum section.

**Verification.**
- ☑ Targeted suites: `python3 -m pytest tests/test_live_momentum_proxy.py tests/test_calibration_report.py tests/test_live_watcher.py -v -k "session_99 or test_paper_trade_persists_estimated or test_paper_trade_omits_proxy"` → 25 passed.
- ☑ Full suite: `python3 -m pytest tests/ --tb=short` → **1605 passed** (Session 98 baseline 1581 + 24 net tests). No new failures.
- ☑ `python3 tools/calibration_report.py --days 7` renders cleanly: vig_stack section unchanged (n=130, Brier 0.4230), new `## live_momentum proxy calibration (n=0)` section with the forward-only caveat as expected pre-restart.
- ☑ Bot restart via Battle Scar #14 path-rooted `launchctl kickstart -k gui/$(id -u)/com.hustle-agent.bot`. Post-restart wrapper + child PID verified, cwd confirmed.

**What did NOT change.**
- No sizing changes (backlog rule: *"First measure calibration; size later."*).
- No entry/exit logic, no threshold tuning, no disabled-sport changes.
- No vig_stack fair-value model touched (different system, different calibration).
- No backfill of historical decisions or paper_trades (forward-only).
- No `predictions.jsonl` integration for live_momentum (preserved as vig_stack-only).
- No `bot/main.py` change.
- No `agent/engine.py` work.

**Docs.**
- `CLAUDE.md` Canonical Data Schema Reference — decisions.jsonl `extra` Session 96 block extended with the 3 new keys; paper_trades.json schema extended with the 3 new live_momentum-only fields. Forward-only readers note added.
- `CLAUDE.md` Data Collection Backlog Priority 3 marked shipped with implementation summary.
- `bot/state/active_observations.json` — Session 99 entry added with 5 metrics: live_momentum decisions carrying estimated_win_prob, paper_trades carrying it, calibration_report renders the proxy section, vig_stack spillover regression-lock (expect 0), Brier watch-list trigger (re-evaluate at 14d / Brier ≤ 0.20 across ≥30 resolved trades).
- README Session 99 recap added.

**Watch-list triggers.**
- **Brier re-evaluation at 14d (target 2026-05-25).** If Brier ≤ 0.20 across ≥ 30 resolved live_momentum trades, the proxy is calibrated enough to consider confidence-weighted sizing in a future session. Brier > 0.25 means worse-than-random — investigate the v1 math identity (likely a `game_context.win_probability` quality issue rather than a proxy bug).
- **Coverage validation.** If 14d post-ship shows < 50% of accepted live_momentum trades carrying `estimated_win_prob`, the wire-up at lines 1422-1453 isn't catching the dominant path — audit which call sites are actually reaching the accept path with `game_ctx` populated. Likely fix: thread `game_ctx` to additional non-watcher entry paths.
- **Model upgrade gating.** Future `_v2` model variants (DQS-weighted blend, momentum tilt) should bump `model_source = "game_context_win_probability_v2"` and ship as a new session AFTER v1 calibration evidence is in. Do NOT silently change v1 weights — calibration analysis will be confused by retroactive model drift.
- **Sizing unblock.** This is the substrate for confidence-weighted sizing (Future Direction lifecycle prerequisite). Sizing changes remain off-limits until v1 calibration is validated. The earliest "size by estimated_win_prob" session can ship is after the 14d Brier evidence accumulates.

---

### ☑ Session 100 — vig_stack ladder context instrumentation (2026-05-11)

**Trigger.** CLAUDE.md "Data Collection Backlog" Priority 5: *"We know which families win, but not enough about which ladder shapes within a family are fragile."* Today's evidence sharpened the gap: discovery agent surfaced three HIGH-severity `tail_loss_in_high_cap_family` alerts; two on KXHIGHAUS within 48 hours (B80.5 and B89.5 NO bets, both -$199 full-notional losses where the day's high landed inside the B-bucket). KXHIGHAUS overall is the strongest family (+$378, 82% WR across 44 trades) — family-prefix granularity can't distinguish "KXHIGHAUS B-bucket rungs near forecast bound" from "KXHIGHAUS overall." Mirrors the Session 99 ship shape (helper module → opp dict enrichment → executor persists → calibration_report extends). Forward-only; no behavior changes.

**Phase 0 — verify the premise before scoping.**

- **0.1 field inventory (key finding).** Parallel Explore agent + direct read of [bot/strategies/vig_stack_series.py](bot/strategies/vig_stack_series.py): 7 of 8 wished-for fields are already computed at decision time and just discarded. `family` at line 606 (`fam = ticker.split("-")[0]`), `ladder_total_yes_sum` as `yes_sum_cents` (line 729), `rung_count` as `ladder["valid_count"]` (line 478), `no_price_cents` as `no_ask_cents` (line 732), `forecast_bucket_distance` via `_forecast_distance_from_bucket()` (line 67) with the NWS cache at `_nws_forecasts` (line 312), `time_to_close_hr` as `tts_hr` (line 654), source metadata via `_SERIES_TO_NWS` map (lines 45-64). Only `selected_rung_rank` needs new code (~10 lines: sort `ladder["valid_tickers"]` by parsed strike, find this opp's index). **Outcome A confirmed** — full ship, no Outcome B/C needed.
- **0.2 persistence path.** Direct read of [bot/executor.py:1170-1207](bot/executor.py:1170): the inject block is NOT a generic prefix loop — it has per-key conditionals (paper_confidence/dqs/sport from Session 50; paper_estimated_win_prob/model_source/confidence_components from Session 99). Each new field requires extending this block. With 13 new keys, the prudent shape is a compact `for k in LADDER_CONTEXT_KEYS:` loop appended after the Session 99 conditionals — keeps existing per-key style untouched while making the new key set a single-file declarative change. Verified: `vig_stack_series` and `vig_stack_futures` opp types both come from the same `VigStackSeries` class; one inject site covers both.
- **0.3 report destination.** Direct read of [tools/calibration_report.py:257-272](tools/calibration_report.py:257): `__main__` unconditionally prints `report()` then `report_live_momentum_calibration()`. Adding `report_vig_stack_ladder_shapes()` as a parallel standalone function + a third unconditional `print()` is the cleanest extension; matches Session 99's slot-in pattern.
- **0.4 test-fixture concern resolved.** [tests/test_vig_stack_series_strategy.py:235-259](tests/test_vig_stack_series_strategy.py:235) `_assert_equivalent()` does subset-equality (iterates over a fixed list of named keys); adding new `paper_*` keys to the opp dict is additive — golden-file fixtures stay valid.
- **0.5 bucket convention.** Direct read of [bot/strategies/vig_stack_series.py:81-103](bot/strategies/vig_stack_series.py:81) `_parse_weather_bucket`: `B89.5` parses to `(89.0, 90.0)` — lo = `float(int(89.5))` = 89.0. "B-bucket" tickers are 1°F-wide ranges starting at the integer floor. The two -$199 KXHIGHAUS losses were `B80.5` (bucket [80.0, 81.0]) and `B89.5` (bucket [89.0, 90.0]); the day's high landed inside.

**Outcome A (ship).** Phase 0 evidence supports the full 8-field ship (with rank as both asc and desc — two ints, lets analysis pivot either way). Plus a version-identifier field (`ladder_context_source`) mirroring Session 99's `model_source` pattern. Total: 13 keys.

**Implementation.**
- **New module [bot/vig_stack_ladder_context.py](bot/vig_stack_ladder_context.py)** (~150 LOC). `compute_ladder_context(ticker, valid_tickers, yes_sum_cents, valid_count, no_ask_cents, forecast_temp, source_city, is_weather, close_ts, now_utc) -> dict` returns 13 un-prefixed keys. `LADDER_CONTEXT_KEYS` tuple at module level is the single source of truth for the executor inject and report. `_parse_strike()` mirrors `_parse_weather_bucket` convention. `MODEL_SOURCE = "vig_stack_ladder_context_v1"`. None values surface for inapplicable fields (futures opps → no rank/strike/kind; non-weather → no forecast fields) so the caller can omit them and avoid dead columns on disk.
- **Wire into accept path at [bot/strategies/vig_stack_series.py:719-743](bot/strategies/vig_stack_series.py:719).** Compute `ladder_ctx_present` once (dict-comp dropping None values), spread un-prefixed into `decisions.log_decision(extra={..., **ladder_ctx_present})` for `accept` decisions, and spread `paper_`-prefixed into the returned opp dict via `**{f"paper_{k}": v for k, v in ladder_ctx_present.items()}`. Single compute, two surfaces. Reject decisions unchanged — they already carry gate-specific extras and enriching them would bloat decisions.jsonl for limited analytical value.
- **Extend executor inject at [bot/executor.py](bot/executor.py).** Top-level `from bot.vig_stack_ladder_context import LADDER_CONTEXT_KEYS`. After the Session 99 conditional block, an 8-line `for k in LADDER_CONTEXT_KEYS: v = opportunity.get(f"paper_{k}"); if v is not None: record[k] = v` loop. The `is not None` guard preserves Session 50/99's spillover invariant — live_momentum opps don't carry `paper_family`/etc, so live_momentum records stay free of ladder fields.
- **Extend [tools/calibration_report.py](tools/calibration_report.py)** with `report_vig_stack_ladder_shapes(days)` reading `bot/state/paper_trades.json` filtered to `type=="vig_stack"` AND `family is not None` AND `status in ("won", "lost")`. Two sections in one function output: **A** per-family × `selected_rung_rank_asc` table (all families; identifies which rungs within a ladder lose) with `(n, won, lost, win_rate, total_pnl, n_lost_last_7d)` columns; **B** per-weather-family × `forecast_bucket_distance` bucket table (`KXHIGH*` / `KXLOW*` only; 5 buckets: deep-in / in / edge / out / far-out) testing the KXHIGHAUS B-bucket hypothesis directly. `__main__` wired with a third unconditional `print()`.

**Tests.**
- **`tests/test_vig_stack_ladder_context.py` (new, 10 tests).** Weather happy-path returns full 13-key dict; non-weather futures opp omits weather + rank fields; missing forecast cache still returns source_city; rank_asc + rank_desc indexing for 3rd-of-5 and 4th-of-5 cases; unsortable futures ladder returns None ranks; unparseable + missing `close_ts` return None `time_to_close_hr`; `MODEL_SOURCE` stability assertion (parallels Session 99 pattern); canonical key list matches returned dict keys (regression-lock against drift); forecast distance sign convention (negative inside, positive outside).
- **`tests/test_vig_stack_series_strategy.py` (3 new tests).** `test_accepted_opp_carries_ladder_context_paper_keys` verifies the opp dict for accepted KXHIGHMIA rungs carries `paper_family`, `paper_rung_count`, `paper_ladder_total_yes_sum_cents`, `paper_selected_rung_rank_asc/desc`, `paper_no_price_cents`, `paper_source_city`, `paper_ladder_context_source` AND that forecast fields are absent when the NWS mock returns `{}`. `test_accept_decision_extra_carries_unprefixed_ladder_context` verifies the un-prefixed mirror lands in `decisions.jsonl` accept extras. `test_reject_decision_extras_unchanged_by_session_100` regression-locks that reject decisions do NOT pick up the Session 100 keys (using the `no_vig` fixture's reject paths).
- **`tests/test_bot_executor.py::TestSession100LadderContextFields` (new, 3 tests).** Persist round-trip: opp carrying all 13 `paper_*` keys lands all 13 renamed fields on paper_trades.json. Spillover regression-lock: live_momentum opp without `paper_family` produces a record with none of the 13 ladder fields. Partial fields: futures opp with only some keys lands only those keys.
...
- No family caps, family floors, or family disable/enable.
- No sizing logic.
- No `live_momentum` work.
- No `predictions.jsonl` integration for vig_stack (still reads from `paper_trades.json` for the new section, same as Session 99's pattern).
- No refactor of existing Session 50 or Session 99 executor conditionals (kept per-key style; new keys use a loop appended after).
- No backfill of historical decisions or paper_trades.
- No `decisions.jsonl` reject-row enrichment (accept-only scope; rejects already carry gate-specific extras).
- No third "per-rung_kind" report sub-table — derivable from per-row data if needed later.
- No KXHIGHAUS B-bucket action (acting on the data is a separate session ≥14d from now per the brief's "Out of Scope" rules).

**Docs.**
- `CLAUDE.md` Canonical Data Schema Reference → paper_trades.json — schema extended with 13 new Session-100 forward-only fields and the "Pre-Session-100 readers must treat absence as None" rule.
- `CLAUDE.md` Data Collection Backlog → Priority 5 marked shipped with implementation summary.
- `bot/state/active_observations.json` — Session 100 entry with 6 metrics: paper_trades records carrying family/rung_count/rank, weather records carrying forecast_bucket_distance, decisions.jsonl accepts carrying the ladder-context extra, calibration_report renders the new section, live_momentum spillover regression-lock (expect 0), KXHIGHAUS forecast-edge loss-concentration watch-list trigger (14d re-eval at 2026-05-25).
- README Session 100 recap added.

**Watch-list triggers.**
- **KXHIGHAUS forecast-edge cohort re-evaluation at 14d (target 2026-05-25).** If post-Session-100 KXHIGHAUS rows show N ≥ 10 in the `[-0.5, 0.5)` forecast_distance bucket AND win_rate < 60%, surface as a candidate for Session 101 forecast-edge-aware floor or sizing decision. Motivating evidence: two -$199 KXHIGHAUS B-bucket losses (B80.5, B89.5) on 2026-05-09 / 2026-05-10 where the day's high landed inside the bucket.
- **Cross-family rank-3 fragility check.** If after 14d Section A shows rank=highest-strike rungs losing > 30% across multiple weather families (KXHIGHAUS + KXHIGHMIA + KXINX), the signal is structural to the ladder shape, not family-specific — promote the question to a per-family-rank floor rather than a per-family floor.
- **Coverage validation at 7d.** If 7d post-ship shows < 50% of accepted vig_stack trades carrying `family` (or any other always-on field), the wire-up at the accept path isn't catching the dominant code path — investigate whether `compute_ladder_context()` is being called consistently.
- **Schema version gating.** Future `_v2` model variants (e.g. adding `intra_ladder_correlation`, `ladder_age`, or `recent_family_pnl`) should bump `ladder_context_source = "vig_stack_ladder_context_v2"` and ship as a new session AFTER v1 evidence accumulates. Do NOT silently change v1 field semantics — downstream analysis will be confused by retroactive drift.
- **Disable-action prerequisite.** No action on KXHIGHAUS or any other family in Session 100. Acting on the data requires (a) ≥14d of post-Session-100 evidence, (b) ≥10 resolved trades in the suspect bucket, (c) a Phase 0-disciplined session that compares the bucket-cohort breakeven WR to its actual WR. Premature action on n=2 anecdotes is the failure mode this session prevents.

### ☑ Session 101 — `_kalshi_get` total wall-clock timeout (2026-05-11, Outcome A re-scoped from audit to surgical infra ship)

**Trigger.** Session 101 was briefed as an audit of `_main_loop` Steps 4–7b, queued by Session 98's "case 3" decision tree: *"degraded cadence + zero `snapshot_outer_timeout_count_24h` → fix can't fire on healthy Kalshi → secondary cause exists."* The brief listed `snapshot_universe` as out of scope based on that inference. Phase 0 evidence overturned the premise — the cause IS `snapshot_universe`; the counter is absent not because Kalshi is healthy but because Session 98's outer guard itself didn't fire. Surfaced contradiction to operator via AskUserQuestion; operator confirmed re-scope to fix `snapshot_universe`-path, Layer 1 only (Layer 2 outer `wait_for` binding fix deferred to a future session).

**Phase 0 — verify the premise before scoping.**

- **0.1 step map.** Parallel Explore agent + direct read of [bot/main.py:1232-1556](bot/main.py:1232): `_main_loop` has 13 numbered steps (1, 2, 3, 3b, 4, 4b, 5, 6, 6b, 7, 7b, 8, 8b, 9). Steps 4–8b are sync wrappers over file-I/O or short Kalshi REST calls bounded at well under 10s; none can plausibly take 1+ hours under documented conditions. Anti-suspect list confirmed.
- **0.2 S95–S100 cross-reference.** S95 (shadow_trades writer) is <1ms per call. S96 (live_momentum context build) runs in `_live_scan_loop`, not `_main_loop`. S99 (live_momentum proxy) runs in `_live_scan_loop`. S100 (vig_stack ladder context) runs inside `scan_cycle()` already wrapped by Session 98's outer guard. All four ships CLEARED. Cadence regression predates them per Session 98's May 3–5 evidence.
- **0.3 forensic log analysis — load-bearing evidence.** Direct read of [bot/logs/bot.log](bot/logs/bot.log) lines 53934 / 54338 / 54349: last SCAN CYCLE before the gap at `[2026-05-11 13:22:19 EDT]`; inner-deadline warning at `[2026-05-11 15:21:42 EDT]` for scan_id `20260511T181751` (18:17:51 UTC = 14:17:51 EDT); next SCAN CYCLE at `[2026-05-11 15:21:57 EDT]`. From scan_id creation to inner-deadline warning: **3831 seconds (1h 3min 51s)**. The 600s outer `wait_for` should have fired at 600s elapsed. It did not. `bot_state.json: snapshot_outer_timeout_count_24h` key absent (Session 98's guard never fired this calendar day). `total_snapshots_today: 88, partial_snapshots_today: 39` → 44% partial rate (4.4× the 10% WARN threshold per CLAUDE.md §12). 25+ partial flushes in last 24h normally bound at ~5min 10s; the 14:17:51 scan was the only outlier at 3831s. A second similar gap (15:21:57 → 17:28:15 EDT, 2h 6min) fired pre-restart while the audit was in progress — chronic condition confirmed.
- **0.4 root cause.** Direct read of [agent/kalshi_client.py:117-141](agent/kalshi_client.py:117): `_kalshi_get` uses `urllib.request.urlopen(req, context=ctx, timeout=10)`. **`urlopen`'s `timeout=` parameter is a per-recv socket timeout, not a total-request timeout.** When Kalshi keeps a TCP connection alive but drips response bytes within the per-recv budget, each `recv()` returns within budget and the timeout never fires. A single hung call can run for hours. The inner deadline check at [bot/universe.py:258, 297](bot/universe.py:258) is between page operations and cannot bound a single hung urlopen call. This matches Battle Scar #13's Session 98 Caveat: *"if `snapshot_outer_timeout_count_24h ≥ 3/day` persists for ≥ 3 days, ship per-request HTTP timeouts in `agent/kalshi_client.py` to bound the leaked work at its source."* The Caveat never triggered because the OUTER guard itself isn't firing, masking the chronic condition.

**Outcome A (re-scoped, Layer 1 only).** User-confirmed via AskUserQuestion. Bound `_kalshi_get` with a daemon-thread total wall-clock cap. Once the inner work is reliably bounded at ≤93s/page (3 retries × 30s + retry sleeps), the inner deadline in `snapshot_universe` becomes the effective ceiling (~310s/snapshot). Layer 2 (outer `wait_for` binding fix — likely `asyncio.get_event_loop()` returning a non-running loop on Python 3.14) is deferred with watch-list trigger; once Layer 1 is in place, the broken outer guard is redundant for cadence purposes.

**Implementation.**
- **[agent/kalshi_client.py](agent/kalshi_client.py):** Added `import threading` and module-level constant `_KALSHI_TOTAL_TIMEOUT_SEC = 30` with explanatory comment citing the 2026-05-11 14:17:51-15:21:42 EDT drip case. Replaced `_kalshi_get`'s in-line `for attempt in range(3): urlopen(...)` with a daemon-thread wrapper: `_worker()` closure runs the original retry logic inside a `threading.Thread(daemon=True, name=f"kalshi-get-{path...}")`; the calling thread `t.join(timeout=_KALSHI_TOTAL_TIMEOUT_SEC)` and raises `TimeoutError(f"_kalshi_get total wall-clock timeout {_KALSHI_TOTAL_TIMEOUT_SEC}s on {path}")` if the worker isn't done. urlopen's per-recv timeout=10 stays as primary defense; the daemon-thread guard is the safety net for trickle. Abandoned workers leak one TCP socket each; under sustained Kalshi degradation we may accumulate a small number (≤120 over 1h of full degradation, well within macOS `ulimit -n`). Daemon=True ensures no block at process exit.
- **No changes to [bot/universe.py](bot/universe.py)** or [bot/main.py](bot/main.py). The new TimeoutError string contains `"timeout"`, which is in `_TRANSIENT_ERROR_TOKENS` at [bot/universe.py:106-113](bot/universe.py:106). `get_markets`'s existing `except Exception as e: return {"error": f"Kalshi API error: {str(e)}"}` wrapper at [agent/kalshi_client.py:212-213](agent/kalshi_client.py:212) catches the TimeoutError and returns the same error-dict shape that `_is_transient_kalshi_error` already matches. `snapshot_universe`'s retry loop at [bot/universe.py:279-307](bot/universe.py:279) handles it identically to a connection-reset. Zero changes downstream of the helper.

**Tests.**
- **[tests/test_kalshi.py](tests/test_kalshi.py) `class TestKalshiGetTotalTimeout` (new, 4 tests).** `test_total_timeout_fires_on_slow_drip` — `_KALSHI_TOTAL_TIMEOUT_SEC` monkeypatched to 0.5s, urlopen patched to `time.sleep(2)`, asserts `TimeoutError("total wall-clock timeout")` raised in < 1.5s. `test_normal_path_unaffected` — urlopen returns a `_MockResponse` context manager with valid JSON; asserts the wrapper returns the parsed dict. `test_propagates_http_429_retry` — urlopen raises `HTTPError(429)` twice then succeeds; asserts the 2s/3s backoff schedule fires inside the worker thread and the eventual response is returned. `test_timeout_message_is_transient_error` — fires the timeout, wraps the message in `f"Kalshi API error: {str(e)}"`, asserts `bot.universe._is_transient_kalshi_error(msg)` returns True (regression-lock the propagation path).
...
**What did NOT change.**
- No behavior changes to `_main_loop` step structure, scan_interval, wall-clock sleep, or any cadence logic.
- No changes to [bot/universe.py](bot/universe.py) — inner deadline check unchanged, retry logic unchanged, page limit unchanged. The fix is upstream in the HTTP helper.
- No changes to [bot/main.py:1290-1315](bot/main.py:1290) Session 98 outer `wait_for` wrap — deferred to Session 102 (see watch-list).
- No strategy behavior changes, no caps, floors, thresholds, disable/enable changes.
- No live_watcher changes; tick loop unaffected.
- No Telegram dedup work; no KXHIGHAUS B-side action.
- No backfill of historical data.

**Docs.**
- This file (CLAUDE-sessions.md) — Session 101 ☑ block added.
- `bot/state/active_observations.json` — Session 101 entry with 4 metrics: longest SCAN CYCLE gap in last 24h (expect <60min during awake hours); median SCAN CYCLE interval (expect ≤35min); count of new `_kalshi_get total wall-clock timeout` log entries (0–10/day under normal Kalshi health, non-zero confirms the guard fires); Layer 2 watch-list trigger.
- README Session 101 recap added.

**Watch-list triggers.**
- **Layer 2 — outer `wait_for` binding investigation (Session 102 candidate).** If `snapshot_outer_timeout_count_24h` in `bot_state.json` remains absent for ≥7 days while the new `_kalshi_get total wall-clock timeout` log line fires non-zero times during the same window, the outer guard at [bot/main.py:1293](bot/main.py:1293) is provably broken (it should fire when the inner guard fires repeatedly enough to overshoot 600s). Open Session 102 to investigate the `asyncio.get_event_loop()` → `asyncio.get_running_loop()` migration (Python 3.14 deprecation hypothesis) and the Session 98 regression test's actual coverage of the production binding scenario.
...
### ☑ Session 110 — Rich live_momentum match-phase: Phase 0 surfaces data, design doc filed (2026-05-12, Outcome B ship)

**Trigger.** S107 (2026-05-11) fixed reachable partial context across scan + executor paths for live_momentum, taking `context_available=true` from ~3.9% (all-history) to ~89% (recent rows), but left a follow-on in CLAUDE.md Open Loops: `match_phase` is structurally thin for IPL and tennis because the regime spec at [bot/regime.py:181-244](bot/regime.py:181) needs sport-specific game state (`over_count`, `set_number`, `round_num`) rather than wall-clock elapsed time. S102 had surfaced IPL as still-profitable for live_momentum (+$53.71 / 13 settled), so cohort observability on `sport × match_phase × leader_strength` was the direct unblock target. Session 110 was scoped per the brief's A (ship field extension) / B (design doc) / C (accept limitation) decision tree, gated on Phase 0 investigation of what sport state is reachable from existing in-scope objects with no new network calls.

**Phase 1 exploration.** Two parallel Explore agents + targeted Read passes mapped the helper anatomy and forward-compat path:
- `_build_live_momentum_decision_context()` at [bot/live_watcher.py:128-258](bot/live_watcher.py:128) is allowlisted by `_LIVE_MOMENTUM_CONTEXT_FIELDS` at [bot/live_watcher.py:69-75](bot/live_watcher.py:69). Local `_context_match_phase(sport, elapsed_seconds)` at [bot/live_watcher.py:108-125](bot/live_watcher.py:108) does elapsed-time buckets for tennis/UFC, returns `None` for IPL.
- **`bot/regime.py:_match_phase(sport, market_state)` already supports state-path semantics for IPL/tennis/UFC.** Docstring at [bot/regime.py:40-44](bot/regime.py:40) explicitly says "the state path is forward-compatible — when a future session sources the rich data, this tagger flips automatically without touching writers." The blocker is upstream data, not the tagger.
- Existing forward-compat TODO marker at [bot/live_watcher.py:1502-1510](bot/live_watcher.py:1502) anchors where rich-state keys should land in `mom_ctx`.
- Empirical sanity check on `bot/state/live_ticks.jsonl` and recent IPL `decisions.jsonl` rows: `espn_scores: null`, `wp: 0.5` (GameContext default), `period: null`, `regime.match_phase: null`, `extra.match_phase: null` — confirming nothing populates `over_count` anywhere today.

**Phase 0 (empirical, in-session).** Probed ESPN cricket and tennis scoreboards directly. Findings:
- The bot's `ESPN_SPORT_PATHS` at [bot/config.py:765-771](bot/config.py:765) contains only `{nba, mlb, nhl, nfl, ncaab}`. **No `ipl`, `atp`, `wta`, or `ufc` mapping.** For these sports `_fetch_espn_score` returns `{}` at the empty-`sport_path` early-return ([bot/live_watcher.py:2707-2714](bot/live_watcher.py:2707)) — the bot has never made an ESPN call for IPL. Earlier exploration-agent claim that ESPN cricket was being hit was incorrect.
- `https://site.api.espn.com/apis/site/v2/sports/cricket/indian-premier-league/scoreboard` returns **HTTP 404**. The slug path is wrong. The correct path is `cricket/8048/scoreboard` (numeric league ID), which returns **HTTP 200** with rich cricket schema.
- Cricket linescore schema (sampled from 2026-05-10 Chennai Super Kings vs Lucknow Super Giants final, second-innings chase): `competitors[].linescores[]` carries `period` (innings), `runs`, `wickets`, `overs` (float, e.g. `19.2` = 19 complete overs + 2 balls), `isBatting`, `isCurrent`. Plus `competitions[0].status.period` (innings). Every regime spec field is directly extractable.
- ESPN tennis (`tennis/atp/scoreboard`, `tennis/wta/scoreboard`) returns tournament-level events without match-level state. `status.period: null` at the tournament competition. Match-level state requires per-tournament event drill-down (the `$ref` pattern visible in cricket `leaders`) — materially more complex than cricket.
- ESPN UFC: not in `ESPN_SPORT_PATHS` family. No live UFC fight state via `site.api`. UFC stays elapsed-only.

**Decision: Outcome B.** Rich state IS reachable for IPL via the documented ESPN endpoint, but enabling it requires adding `"ipl": "cricket/8048"` to `ESPN_SPORT_PATHS`, which activates a per-tick HTTP fetch the bot has never made. That violates two of the session's discipline constraints:
1. The S96/S107 "no new network calls in the decision path" hard constraint.
2. The S110 brief's explicit "No ESPN integration, no cricket API" out-of-scope rule.

Phase 0 produced concrete evidence that the integration is feasible and the schema fits regime.py's contract exactly — but the integration itself is exactly the kind of focused, scoped work that deserves its own session with full HTTP-overhead validation, off-season behavior probing, and watch-list trigger discipline. Outcome A is therefore deferred-to-a-follow-on, not killed. Outcome C is too strong because the data path is concrete. Tennis remains deferred behind ATP/WTA re-enable and per-match drill-down complexity.

**Outcome B shipped.**
- [docs/superpowers/specs/2026-05-12-live-momentum-rich-match-phase-design.md](docs/superpowers/specs/2026-05-12-live-momentum-rich-match-phase-design.md): full design doc covering Phase 0 findings, the IPL ESPN integration blueprint (config change + parser branch + helper kwargs + `mom_ctx` threading + tests), tennis/UFC/NHL out-of-scope reasoning, cost estimate (~17,280 fetches/day per match-day, same profile as NHL ESPN), and the discipline constraints the follow-on session must respect (re-validate endpoint, verify off-season behavior, spillover regression-lock).
- [CLAUDE.md](CLAUDE.md) "Open Loops" → "Blocked: needs dedicated session to unblock data collection": replaced the brief description of the S107 follow-on entry with a pointer to the design doc + crisp summary of why the integration was deferred from S110 (constraint conflict, not data gap).
- [bot/state/active_observations.json](bot/state/active_observations.json): new Session 110 entry — Outcome B; no observable runtime metric until the follow-on integration ships; the 30-day re-check is "has the follow-on session run yet?", not a metric on bot behavior.

**Tests.**
- ⚠ Pre-ship baseline: `python3 -m pytest tests/ --tb=short -q` → **1639 passed, 1 failed in 41.97s**. The one failure is `tests/test_glint_status.py::test_consolidator_completes_in_under_2s` (elapsed 2.16-2.22s vs. 2.0s budget, consistent across 3 consecutive runs — NOT a flake). The test reads REAL state files at REPO_ROOT (`decisions.jsonl`, `paper_trades.json`, `clv.json`, `universe.jsonl`); their natural growth between the S109 ship report (1640/0) and S110 start (24h later) nudged elapsed past the 2.0s budget. **Not introduced by S110** — zero `bot/` or `tests/` touches in this session (Outcome B is docs-only). Tried a threshold bump (`<2.0s` → `<3.0s`), explicitly denied by operator as out-of-scope per the brief: "Test changes that adjust assertions to match degraded behavior." Bump reverted; failure surfaced as an Open Loops follow-up entry (`Consolidator perf regression`) rather than hidden via assertion adjustment. The honest read: this is a test-fixture-design issue (non-deterministic test using live REPO_ROOT state), surfaced by natural growth; future direction is either (a) optimize `build_snapshot` hot paths in `tools/glint_status.py:1634` or (b) refactor the test to seed a minimal-fixture REPO_ROOT in a tmpdir.
- ☑ Post-ship: `python3 -m pytest tests/ --tb=no -q` → **1639 passed, 1 failed** (same single failure; baseline preserved minus the pre-existing consolidator-budget regression).
- ☑ JSON validation: `python3 -m json.tool bot/state/active_observations.json`.
- ☑ Production-code touch check: `git diff --stat -- bot/ tests/` shows zero changes under `bot/` or `tests/` — only the docs paths, CLAUDE.md, CLAUDE-sessions.md, README.md, and force-added `bot/state/active_observations.json`.
- ☑ No restart (no runtime change).

**What did NOT change.**
- No bot code touched. `bot/live_watcher.py`, `bot/regime.py`, `bot/config.py`, and `tests/` are untouched.
- `ESPN_SPORT_PATHS` is unchanged — adding `"ipl"` is exactly the deferred work.
- No entry / exit / sizing / disable changes to live_momentum.
- No vig_stack work.
- No tennis instrumentation changes.
- No NHL `regime._MATCH_PHASE_SPORTS` expansion (separate session entirely).
- No historical backfill (forward-only per S103/S108 discipline).
...
### ☑ Session 112 — Ship IPL ESPN integration: S107 follow-on resolved (2026-05-11, Outcome A ship)

**Trigger.** S107 (2026-05-11) shipped reachable partial context across the live_momentum scan + executor paths and noted in CLAUDE.md Open Loops that `match_phase` was still structurally null for IPL because `regime._match_phase("ipl", market_state)` reads `over_count` from `market_state`, and the bot has never sourced cricket state. S102 (2026-05-11) had surfaced IPL as the second still-profitable live_momentum cohort post-S97 (+$53.71 / 13 settled — only IPL and NHL net positive after the disable round). S110 (2026-05-12) ran Phase 0 against `https://site.api.espn.com/apis/site/v2/sports/cricket/8048/scoreboard`, confirmed full innings + over_count + wickets schema (HTTP 200, ~12-21 KB), and filed Outcome B because shipping the integration would have violated the S96/S107 "no new network calls" rule and the S110 brief's explicit "no ESPN integration, no cricket API" without an operator decision. Session 112 is that operator decision: a focused, IPL-scoped relaxation that ships the design doc's blueprint and bounds future relaxation explicitly in CLAUDE.md.

**Phase 0 re-verification.** Re-curled `cricket/8048/scoreboard` ahead of the work: HTTP 200, 12,447 bytes, 56 ms. Today's IPL match `Gujarat Titans v Sunrisers Hyderabad` was scheduled for 19:30 IST and appeared in the response with `events[0].status.type.state = "pre"`. Endpoint is alive and serving the documented schema; the design doc's live-game shape (sourced from the 2026-05-10 CSK-LSG sample) is the canonical reference for parser construction. No endpoint drift since S110.

**Decision: Outcome A.** Endpoint verified, integration surface small (one config dict entry + one parser branch + context plumbing + match_phase dispatcher delegation), graceful-degrade failure modes already in place (`_fetch_espn_score` returns `{}` on any exception; `_LIVE_MOMENTUM_CONTEXT_FIELDS` allowlist + `missing_context_fields` annotation already enforces S103/S108 no-fabrication discipline). Per-request `urlopen(..., timeout=8)` consistent with existing NBA/MLB/NHL/NFL/NCAAB. Existing 3-tick (~30s) throttle at [bot/live_watcher.py:1380-1391](bot/live_watcher.py:1380) is sufficient caching since over_count changes once per over (~3-4 min in T20 IPL).

**Outcome A shipped (7 code/doc surfaces, ~100 production LOC + ~320 test LOC):**

- [bot/config.py:765-779](bot/config.py:765): added `"ipl": "cricket/8048"` to `ESPN_SPORT_PATHS` with a 6-line comment naming the operator relaxation and pointing to the Network Call Policy section.
- [bot/live_watcher.py:69-78](bot/live_watcher.py:69): extended `_LIVE_MOMENTUM_CONTEXT_FIELDS` allowlist with `over_count`, `innings`, `wickets`, `runs_scored` (forward-only, S96/S99/S100 pattern).
- [bot/live_watcher.py:108-133](bot/live_watcher.py:108): added `over_count` kwarg to `_context_match_phase` and an IPL state-path branch that delegates to `bot.regime._match_phase("ipl", {"over_count": over_count})` so the canonical powerplay / middle / death bucket boundaries stay single-sourced at [bot/regime.py:236-244](bot/regime.py:236). Tennis / UFC time-path branches unchanged.
- [bot/live_watcher.py:215-271](bot/live_watcher.py:215): extended `_build_live_momentum_decision_context` — reads `over_count`, `innings`, `wickets`, `runs_scored` from `espn_data` and includes them in the returned context dict. Existing `compact = {k: v for k, v in context.items() if v is not None}` filter drops None values, so non-IPL rows stay clean while cricket field names still appear in `missing_context_fields` on IPL rows where ESPN data is unavailable.
- [bot/live_watcher.py:1497-1512](bot/live_watcher.py:1497): updated `mom_ctx` to thread `over_count` from `espn_data` so `regime.tag` populates `decisions.regime.match_phase` for IPL (in addition to the `extra.match_phase` populated by the context builder). Refreshed the Session-34 forward-compat comment to reflect S112's state-path activation.
- [bot/live_watcher.py:2762-2818](bot/live_watcher.py:2762): extended `_fetch_espn_score` with a cricket extraction block — when `self.sport == "ipl"`, walks `home_linescores` / `away_linescores`, finds the entry where `isBatting AND isCurrent`, and extracts `over_count = max(1, min(20, int(overs) + 1))`, `innings`, `wickets`, `runs_scored`. Cricket fields spread into the return dict only when present (S103/S108 no-fabrication). Non-IPL sports skip the block entirely so their return shape is bit-for-bit unchanged (spillover regression-locked).
- [tests/test_live_watcher.py:147-499](tests/test_live_watcher.py:147): 14 new TDD tests under a "Session 112: IPL ESPN cricket integration" header. Parser tests (`_fetch_espn_score_cricket_*`) stub `urllib.request.urlopen` with hand-constructed fixtures shaped per the S110 design doc CSK-LSG sample. Context-helper tests (`_live_momentum_context_helper_ipl_*`) hit the public API directly. Match-phase tests (`_context_match_phase_*`) verify the regime delegation. Two spillover regression-locks: `_fetch_espn_score_nba_unchanged_shape` (parser-layer) and `_live_momentum_context_helper_nba_no_cricket_spillover` (context-layer).
- [CLAUDE.md "Network Call Policy"](CLAUDE.md): new section under Project Scope; names the bounded relaxation (IPL only) and explicitly disclaims general permission for future sport additions.
- [CLAUDE.md "Open Loops → Blocked"](CLAUDE.md): replaced the S107 follow-on entry with a one-line "Resolved by S112" note and cross-reference to the Network Call Policy section.
- [CLAUDE.md "Canonical Data Schema Reference" → Session 96 live_momentum subsection](CLAUDE.md): added the four cricket fields to the type listing with sourcing rules (sport=ipl AND current-batting linescore) and forward-only language (pre-S112 IPL rows absent the keys).
- [bot/state/active_observations.json](bot/state/active_observations.json): Session 112 entry with 14-day window and three metrics (ESPN cricket call success rate; IPL row coverage of `over_count` field; spillover check on non-IPL paths).

**Tests.**
- ☑ S112 targeted: `python3 -m pytest tests/test_live_watcher.py -v -k "S112 or cricket or ipl or _context_match_phase or _no_cricket or unsupported_sport or nba_unchanged"` → **14 passed in 0.11s**.
- ☑ Full pytest: `python3 -m pytest tests/ --tb=short -q` → **1654 passed, 0 failed in 38.90s**. S109 / S111 baseline (1640/0) preserved exactly — +14 new tests, 0 regressions.

**What did NOT change.**
- No entry, exit, sizing, disable, or threshold behavior. Pure context enrichment.
- No `vig_stack_series` / `vig_stack_futures` paths. Spillover regression-locked.
- No NBA / MLB / NHL / NFL / NCAAB ESPN parsing changes. Spillover regression-locked at parser-layer (`test_fetch_espn_score_nba_unchanged_shape`) and context-layer (`test_live_momentum_context_helper_nba_no_cricket_spillover`).
- No new helper module. Cricket extraction lives inline in `_fetch_espn_score` alongside the existing `_PERIOD_LABELS` per-sport pattern; promoting to a helper would be premature with one call site and ~30 LOC.
- No per-request timeout semantics change on `_fetch_espn_score`. Existing `urlopen(req, ..., timeout=8)` retained — weaker than S101's daemon-thread wall-clock guard but consistent with the existing 5 ESPN-backed sports. Promoting all ESPN calls to S101-style is a separate operational-hygiene session if the post-restart 14-day observation surfaces stalls.
- No `regime._match_phase` IPL implementation change. Buckets (powerplay overs 1-6, middle 7-15, death 16-20) already correct since Session 34; `_context_match_phase` now delegates rather than duplicating.
- No general constraint relaxation. The "no new network calls" rule still applies to tennis, UFC, NHL `match_phase`, and any future sport — each requires its own approved exception. The CLAUDE.md "Network Call Policy" section is load-bearing for this.
...
**Watch-list trigger (14d, 2026-05-25):** registered in `bot/state/active_observations.json` Session 112 entry. Re-measure (a) ESPN cricket call success rate ≥90% during IPL match windows, (b) `over_count` populated on ≥80% of post-restart IPL rows during live IPL matches, (c) zero spillover of cricket fields onto non-IPL decisions. If (a) < 90% → investigate ESPN endpoint stability (likely league ID rotation). If (b) < 80% → investigate parser team-name matching (Kalshi ticker suffix vs ESPN displayName). If (c) > 0 → revert immediately and investigate spread-syntax leakage.

**Architectural lesson.** The S96 / S99 / S100 forward-only schema pattern + `missing_context_fields` allowlist composes cleanly with sport-specific field additions: register fields in the allowlist, populate from `espn_data`, let the existing compact filter drop None values, let the existing missing-fields annotation surface absences on rows where ESPN data didn't reach the helper. Future sport-specific state extensions (when each gets its own approved network-call exception) can follow the same shape with no schema changes.

### ☑ Session 113 — Disable live-game Telegram card editing (2026-05-11, Outcome A ship — config flag)

**Trigger.** CLAUDE.md Open Loops "Operational hygiene" had a long-running entry about Telegram `edit_message_text` "Message is not modified" dedup spam claimed at ~236/hour and escalating since 2026-05-10. The Session 113 brief reframed it: operator clarified during planning that they don't actually read Telegram, which made the entire live-game card subsystem — one `editMessageText` call per 10s tick per active `LiveGameWatcher` — pure cost without value. The reframing inverted the goal: the error rate was always secondary; the real change is "stop spending API calls on a channel nobody watches." Keep event-driven Telegram paths (trade confirms, daily summaries, slash commands) alive because they're low-volume and useful if the operator ever checks.

**Phase 0 corrections — surfaced during exploration, recorded here so future sessions don't inherit the wrong framing.**

1. **The "236/hour" error rate framing is wrong by ~three orders of magnitude.** Direct measurement against `bot/logs/bot.log.1` (window 2026-05-08 23:36:31 → 2026-05-11 21:55:14, ~70.3 hours, 10 MB rotated): **42 raw "Message is not modified" lines = ~0.6/hour.** Each error event generates 3 lines (2× transient-failure WARN + 1 final ERROR via the Session-52 retry loop), so unique failing edit attempts are ~0.2/hour. Current `bot/logs/bot.log` since 2026-05-11 22:54:36 had 0 hits even before the disable. The 236/hour figure may have been transcribed from a different metric or a different window — the Open Loops entry's framing was never accurate. **The disable still makes sense on operator-doesn't-read-Telegram grounds; the error-rate reduction is a small co-benefit, not the primary driver. Do NOT claim the dedup logic was "fixed" — it was bypassed; the bug at [bot/notifier.py:993](bot/notifier.py:993) (hash store happens only after a successful edit, so identical-content failures retry forever) is untouched.**
2. **`send_alert()` at [bot/notifier.py:1030](bot/notifier.py:1030) is dead code.** `grep -rn "send_alert" bot/` returns only the definition itself — zero callers. The "opportunity buttons" subsystem the brief referenced doesn't fire. No preservation work needed for it; flagged here in case a future session intends to re-enable opportunity-button workflows.
3. **Event-driven Telegram paths are isolated from the live-card subsystem.** `send_confirmation` ([bot/notifier.py:1062](bot/notifier.py:1062)), `send_resolution` ([bot/notifier.py:1067](bot/notifier.py:1067)), `send_daily_summary` ([bot/notifier.py:1072](bot/notifier.py:1072)), and `_handle_text` slash-command dispatch all route through `send_message()` ([bot/notifier.py:944](bot/notifier.py:944)) and the base `_telegram_call()` retry/flood wrapper — they do NOT touch `edit_message_by_id` or `send_message_get_id`. Cross-cutting infrastructure (`_telegram_call` retry/flood) is shared but only at the HTTP-protocol level (no state coupling). Disabling live-card calls cannot break event-driven paths.

**Decision: Outcome A.** Config flag + two notifier-side guards. Minimal-diff: the cascade design uses existing `if self.status_msg_id:` guards at [bot/live_watcher.py:2205](bot/live_watcher.py:2205) and [bot/live_watcher.py:2633](bot/live_watcher.py:2633), so no `bot/live_watcher.py` edits are needed.

**Outcome A shipped (3 production files + 1 state file, ~30 production LOC + ~80 test LOC):**

- [bot/config.py:807-819](bot/config.py:807): added `LIVE_GAME_CARDS_ENABLED = False` (default False per operator preference) with a 9-line rationale comment matching the `CRYPTO_ENABLED` style. Placed between the Crypto Monitoring and Scheduled Briefings sections.
- [bot/notifier.py:30-36](bot/notifier.py:30): refactored the `from bot.config import ...` line into a parenthesized multi-line import with `LIVE_GAME_CARDS_ENABLED` added (alphabetical: `BOT_STATE_FILE, LIVE_GAME_CARDS_ENABLED, PAPER_MODE, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID`).
- [bot/notifier.py:970-975](bot/notifier.py:970): early-return guard at the top of `edit_message_by_id` — returns `False` when the flag is False, before any `_check_flood` / `_wait_for_edit_slot` / dedup work runs. Flag check is FIRST so the no-op holds regardless of `self.app` state.
- [bot/notifier.py:1003-1006](bot/notifier.py:1003): same shape on `send_message_get_id` — returns `None` when the flag is False. The cascade then flows through [bot/live_watcher.py:940 / 981](bot/live_watcher.py:940) assigning `None` to `self.status_msg_id`, after which the existing `if self.status_msg_id:` guards at [bot/live_watcher.py:2205 / 2633](bot/live_watcher.py:2205) short-circuit per-tick edits naturally — zero caller-side edits.
- [tests/test_notifier.py](tests/test_notifier.py): 4 new pytest cases in the existing `asyncio.run()` + `SimpleNamespace` + `FakeBot` style (no `AsyncMock` — followed the project's existing pattern):
  - `test_edit_message_by_id_noop_when_live_cards_disabled` — flag False → returns False, `FakeBot.edit_message_text` never called.
  - `test_send_message_get_id_noop_when_live_cards_disabled` — flag False → returns None, `FakeBot.send_message` never called.
  - `test_edit_message_by_id_works_when_live_cards_enabled` — flag True regression-lock: original `edit_message_text` call still fires.
  - `test_send_message_preserved_when_live_cards_disabled` — validates Phase 0 finding that event-driven `send_message` is unaffected by the flag (locks in that trade confirms / summaries / slash commands keep working).
- [tests/test_notifier.py](tests/test_notifier.py): 3 existing tests updated to `monkeypatch.setattr(notifier, "LIVE_GAME_CARDS_ENABLED", True)` so they continue exercising the actual code path (rather than hitting the new no-op guard): `test_edit_dedup_skip_identical_content`, `test_edit_dedup_records_only_on_success`, `test_high_edit_volume_does_not_exceed_throttle`. These still validate the underlying dedup + throttle machinery for the case where the operator flips the flag back on.
- [bot/state/active_observations.json](bot/state/active_observations.json): Session 113 entry with 14-day window and 4 metrics (editMessageText call volume = 0; "Message is not modified" error count = 0; event-driven path liveness; 14d re-evaluation trigger). Force-added per state-file convention.
- [CLAUDE.md "Open Loops → Operational hygiene"](CLAUDE.md): removed the "Telegram `edit_message_text` 'Message is not modified' dedup spam" bullet. Resolved by disable, not by dedup fix.

**Tests.**
- ☑ Notifier suite: `python3 -m pytest tests/test_notifier.py -v` → **21/21 passed** (4 new + 17 existing).
- ☑ Full pytest: `python3 -m pytest tests/ --tb=short` → **1658 passed, 0 failed in 59.36s**. S112 baseline (1654/0) + 4 new tests = 1658/0. Zero regressions.

**What did NOT change.**
- No `bot/live_watcher.py` edits. The cascade through the existing `if self.status_msg_id:` guards was intentional and is the load-bearing minimal-diff property.
- No removal of any live-card code. The flag-and-guard approach keeps the option to flip the flag back on without further code work (`LIVE_GAME_CARDS_ENABLED = True` in `bot/config.py` is the entire toggle).
- No dedup-logic fix at [bot/notifier.py:993](bot/notifier.py:993). The hash-store-only-on-success bug is bypassed (calls don't fire), not fixed (the logic is unchanged). If the operator ever flips the flag back on, the same dedup behavior returns. That's an intentional discipline call per the brief: "no fabrication of fixes for dedup logic."
- No event-driven Telegram path changes. `send_confirmation`, `send_resolution`, `send_daily_summary`, slash command handlers, and `_telegram_call` retry/flood machinery are bit-for-bit identical. Regression-locked at [tests/test_notifier.py](tests/test_notifier.py) by `test_send_message_preserved_when_live_cards_disabled`.
- No `send_alert` change (Phase 0 found it dead code; no point in disabling something that doesn't fire).
- No strategy / sizing / scanner changes. Live-game watchers still spawn, still tick every 10s, still run trade logic — they just skip the Telegram update step.
...
- ✅ `grep -c "edit_message_text\|edit_message_by_id" bot/logs/bot.log` → **0** post-restart.
- ✅ `grep -c "Message is not modified" bot/logs/bot.log` → **0** post-restart.
- ✅ `tail -500 bot/logs/bot.log | grep -E "ERROR|CRITICAL"` matching notifier/live_watcher → **none**.
- ⏳ Operator-discretionary `/status` Telegram spot-check NOT performed. Brief explicitly flags this as operator-discretionary; the regression test at unit-test time (`test_send_message_preserved_when_live_cards_disabled`) covers the contract.
...
### ☑ Session 115 — Substrate maturity review: 7 forward-collecting observations re-read (2026-05-12, Outcome B — documentation only)

**Trigger.** After 7 consecutive substrate-fix ships (S107 context → S114 dead-zone verify) across 2026-05-11/12, the bot's data substrate is mature; the use of that data is not. The brief framed Session 115 as the analytical counterpart to the shipping streak — read what the substrate has actually produced, decide what (if anything) to do next. S102's parallel-sub-investigation shape on pre-S107 substrate was the precedent.

**Methodology.** 7 sub-investigations dispatched in parallel (S84/S89 pattern), each with a self-contained data source / filter / computation / verdict rule. Discipline: empirical Ns only — no fabrication, no "as of S102" hand-waves, conservative on actionability, default Outcome B if findings split. S103 / S108 / the 400× framing-error lessons remained the active reminder throughout. Bot continued at PID 85470 (S113 restart); no restart needed.

**Sub-Investigation 1 — S99 live_momentum Brier calibration.** Filter on [bot/state/paper_trades.json](bot/state/paper_trades.json): `type == "live_momentum"` AND `estimated_win_prob` present AND `status` settled (not `cancelled_stale`, null `pnl` excluded). **N = 2** (PAPER-BAA84AF2 IPL @ pred=0.500 lost; PAPER-54CA70B8 NBA @ pred=0.808 won). Brier = 0.1434, Bias = 0.1540. Far below the N≥30 watch-list threshold. Both rows source from `game_context_win_probability_v1` with `pred == confidence_components.pre_clamp_raw` (unclamped). No anomalies (no all-zero/all-one degenerate distribution); the 108 remaining live_momentum paper_trades rows pre-date S99 instrumentation (forward-only). **Verdict: WAIT** (re-eval 2026-05-25 as scheduled).

**Sub-Investigation 2 — S100 KXHIGHAUS B-bucket forecast-edge.** Filter on [bot/state/paper_trades.json](bot/state/paper_trades.json): `type == "vig_stack"` AND `family == "KXHIGHAUS"` AND `forecast_bucket_distance` present AND ticker contains `-B` AND `status` settled. **N in [-0.5, 0.5] bucket = 0** (total qualifying = 0; only 2 KXHIGHAUS-B rows carry the new S100 fields, both still `open` with `forecast_bucket_distance` ≥ 3.0 — well outside the watch-list bucket). The two motivating losses are confirmed by ticker/pnl: `KXHIGHAUS-26MAY08-B80.5` pnl=-$199.20 and `KXHIGHAUS-26MAY09-B89.5` pnl=-$199.87, both with `family = null` AND `forecast_bucket_distance = null` — pre-S100 instrumentation, so they cannot be bucket-classified retroactively. The trigger condition (N≥10 in bucket AND WR<60%) is structurally un-evaluable until forward data accumulates. **Verdict: WAIT** (re-eval 2026-05-25 as scheduled).

**Sub-Investigation 3 — S107 live_momentum context coverage.** Filter on [bot/state/decisions.jsonl](bot/state/decisions.jsonl): `opp_type == "live_momentum"` AND `ts >= "2026-05-11T22:00:00Z"`. **N = 20, context_available coverage = 100.0%** — ceiling, well above the 85% threshold. Per-sport breakdown: atp=11, atp_challenger=2, nba=6 (sic — label-drift sub-finding below), wta_challenger=1. **Zero IPL / NHL / UFC decisions in the window** — no enabled-sport live games surfaced since S107 ship. IPL match_phase distribution is therefore empty; the joint per-IPL-phase analysis with Sub-Investigation 6 cannot yet be teed up. **Sub-finding (label drift, not actionable today):** `atp` (11) and `nba` (6) appear in the cohort despite `MOMENTUM_DISABLED_SPORTS = {atp, atp_challenger, nba_game, wta, wta_challenger}`. These are sport_disabled rejects, correctly being context-instrumented per S107's "reachable partial-context paths" widening; the `nba` vs `nba_game` discrepancy is a label vs ticker-prefix vs ESPN-label mismatch worth a 5-minute sanity check in a future session, NOT a S97 enforcement regression. **Verdict: HEALTHY** (substrate working; IPL-slice readiness waits on a live match window).

**Sub-Investigation 4 — S108 reentry breaker counterfactual.** Filter on [bot/state/shadow_trades.jsonl](bot/state/shadow_trades.jsonl): `blocked_reason == "reentry_blocked"` AND `would_contracts != null` AND `sizing_status == "available"` AND market settled. **N_settled = 0; total_would_pnl = $0.00.** Total reentry_blocked rows post-S108 = 2 (both IPL `KXIPLGAME-26MAY11DCPBKS-PBKS`, yes-side @ 0.68 / 0.67, logged 2026-05-11); both carry `sizing_status = "unavailable"` and null `would_contracts` / `would_notional`. **Sub-finding (substrate concern, not actionable today):** S108's live_momentum sizer wire-up did not populate contracts on these rows. Per Schema spec, unavailable rows should carry `extra.missing_sizing_fields` / `extra.sizing_unavailable_reason` for diagnostic transparency — verify on the next live_momentum reentry_blocked row whether annotations are present; if structurally missing, a small instrumentation patch is warranted. The 0d-since-S108-ship window means this is also the explicit "very early after ship" case the brief anticipated. **Verdict: WAIT** (14d window opens ~2026-05-26).

**Sub-Investigation 5 — S109 post-event reversion re-measure.** Source: [bot/state/clv.json](bot/state/clv.json) + universe data. S106 methodology extracted: events = absolute mid-cents move ≥ event_threshold between adjacent liquid (`volume_24h ≥ 10000`) universe snapshots within 180m; reversion = post-event mid returning past pre-event mid within the observation window. S109's 1c-threshold reference: **16.6% reversion (34/205) within 6h**, with reversion rate *decreasing* as move size grows (2c=13.5%, 3c=12.3%) — directional finding is **persists, not reverts**. **Post-S109 ≥5c events all-families = 395; KXCOPPERD + KXAAAGASD only = 0.** Target families had universe coverage ending 2026-05-11 17:20–19:21Z, prior to S109 ship 2026-05-12T00:04:36Z; zero target-family ≥5c events at hour-3 of the 30-day window. **Supplementary all-families read:** 43.5% reversion of eligible / 32.2% of all 395 events — directionally *above* the 40% bar on the eligible-only read. The watch-list is scoped to KXCOPPERD+KXAAAGASD per S106's design rationale; this all-families supplement is interesting but does NOT gate the decision today. **Verdict: WAIT** (calendar trigger 2026-06-11 remains authoritative).

**Sub-Investigation 6 — S112 IPL cricket-state hydration.** Filter on [bot/state/decisions.jsonl](bot/state/decisions.jsonl): `opp_type == "live_momentum"` AND `extra.sport == "ipl"` AND `ts >= "2026-05-11T22:00:00Z"`. **N = 0.** All 11 IPL live_momentum rows in `decisions.jsonl` are timestamped 2026-05-10T16:19 → 2026-05-11T17:27 — pre-S112 ship cutoff (22:00Z). Bot is actively logging post-cutoff (most recent decision 2026-05-12T03:35:37Z, with non-IPL live_momentum rows for ATP/WTA/NBA emitted normally), so the cricket-fields gap is not a substrate failure — there has simply been no live IPL match window since the ESPN cricket/8048 integration shipped. **Verdict: WAIT** (re-measure when next live IPL window arrives; combine then with Sub-Investigation 3 for joint per-IPL-phase analysis).

**Sub-Investigation 7 — S113 Telegram error rate verification.** Grep on `bot/logs/bot.log{,.1}` for `"Message is not modified"` post-S113 ship time (2026-05-11T22:54:36Z; ~5h elapsed measured against UTC sample 2026-05-12T03:51:39Z). **post-ship count = 0; rate_per_hour = 0.00.** Cross-check against the corrected **~0.6/hour baseline** (42 pre-ship raw lines in bot.log.1 over a 70.3h window — NOT the 236/hour erroneous figure that propagated for days). All 42 historical hits are pre-ship (last 2026-05-11 05:30:46, file ends 21:55:14). Bot was actively logging post-ship (2136 lines between 22:54:13 → 23:53:42 — momentum ticks, telemetry, Telegram getUpdates polling normally). Notably `editMessageText` calls continued for ~12 min post-ship (236 calls 22:54:14 → 23:05:59, all HTTP 200) and then ceased completely — consistent with the flag taking effect on the next live-card render cycle. Zero residual "Message is not modified" errors across the full post-ship window. **Verdict: HEALTHY** (S113 disable working as designed; no residual call site to chase).

**Sub-Investigation 8 — Synthesis.**

Verdicts:
- **HEALTHY (2):** S107 (context coverage at 100% with the patch operative on the disabled-sport-reject cohort), S113 (Telegram editMessageText disable producing 0 errors against the ~0.6/hr baseline).
- **WAIT (5):** S99 (N=2 of 30), S100 (N_bucket=0; instrumentation 1d old), S108 (N_settled=0; instrumentation 0d old), S109 (target-families N=0 of 50 at hour-3 of a 30-day window), S112 (no live IPL window since ship).
- **ACTIONABLE / KILL-EARLY (0):** none.

Cross-finding dependency: Sub-Investigation 3 (S107 IPL coverage N=0) + Sub-Investigation 6 (S112 cricket-fields N=0) jointly block the queued per-IPL-phase analysis until a live IPL window surfaces. Both are correctly in WAIT, not ACTIONABLE-REGRESSION — the substrate is in place and will populate when data arrives.

**Three sub-findings to surface but NOT to act on today:**
1. **S107 atp/nba label drift** — `atp` (11) and `nba` (6) labels appear in post-S107 decisions despite the S97 disable set. Likely a `nba` vs `nba_game` ticker-prefix vs ESPN-label discrepancy at the sport-tagging site, NOT an enforcement regression (these are sport_disabled rejects, not accepts). Verify in 5 min the next time a sport-tagging or shadow-trade investigation is in scope; do not open a session for this alone.
2. **S109 supplementary all-families reversion at 43.5%** — above the 40% promote threshold on the eligible-only read at hour 3. The watch-list is scoped to KXCOPPERD+KXAAAGASD per S106's design rationale; the all-families read is supplementary and must NOT widen the watch scope unilaterally. Re-examine at the 2026-06-11 calendar trigger; if the all-families pattern holds and the target-family pattern is still N-thin, that's the moment to discuss whether to widen S106's family list.
3. **S108 sizing wire-up missing diagnostic metadata** — the 2 reentry_blocked rows carry `sizing_status="unavailable"` with null contracts/notional, but appear to lack the `extra.missing_sizing_fields` / `extra.sizing_unavailable_reason` annotations the Schema spec requires for transparency. Verify next time a reentry_blocked row fires; if structurally missing, ship a small follow-on patch when the next instrumentation session is in scope.

**Decision: Outcome B — substrate matured but findings mixed.** Two HEALTHY confirmations validate the S107/S113 ships' real-world effects; five WAITs are honest reflections that those observation windows have only had 0–3 days of forward accumulation, far short of the 14–30 day re-evaluation gates. **No next-session prompt queued** — the 5 WAITs have explicit calendar triggers (S99 2026-05-25, S100 2026-05-25, S108 ~2026-05-26, S109 2026-06-11, S112 2026-05-25 OR N≥20 IPL rows) that auto-fire when ready. **Pattern C count: 16 → 16** (this session opens no new no-fix declaration; it is a scheduled substrate snapshot, not an investigation of a specific question).

**Outcome B shipped (3 documentation surfaces, 0 production files, 0 test LOC):**
- [CLAUDE-sessions.md](CLAUDE-sessions.md) — this Session 115 ☑ block.
- [bot/state/active_observations.json](bot/state/active_observations.json) — refreshed `current_value` and `last_updated` on the watch-list-trigger metric for each of S99 / S100 / S107 / S108 / S109 / S112 / S113 (7 entries touched). No entries added, removed, or superseded.
- [README.md](README.md) — added Session 115 changelog entry following the S114 paragraph-pattern; links to this CLAUDE-sessions.md block.

CLAUDE.md "Open Loops" was deliberately NOT touched — no finding moves any deferred-item status. S106 / S105 / S101-Layer-2 / IPL-S112 entries retain their existing triggers.

**Tests.** None added or modified — no production code touched. Pre-session pytest baseline (1658/0 from S113) is preserved by structural identity.

**What did NOT change.**
- No `bot/` code edits.
- No new constants, no threshold tuning, no disabled-sport/family list changes.
- No bot restart (PID 85470 continues; verified at session start with `etime 45:35` and again at session-end).
- No `tests/` files. No `bot/state/positions.json` / `paper_trades.json` / `decisions.jsonl` / `shadow_trades.jsonl` / `clv.json` writes — only the operator-curated `active_observations.json` was touched, and only `current_value` / `last_updated` fields.
- No new instrumentation. The 7 forward-collecting observations are unchanged.

**Verification.**
- ✅ Every claim in the block cites an empirical N from a fresh sub-agent read against the actual file — zero "as of S102" or "from memory" hand-waves (S103 / 400× framing-error lesson respected).
- ✅ `git diff --stat` confirms changes confined to `CLAUDE-sessions.md`, `bot/state/active_observations.json`, `README.md`. Zero `bot/` source files. Zero `tests/` files. Zero state writes outside the operator-curated registry.
- ✅ `grep -n "^### ☑ Session 115" CLAUDE-sessions.md` returns exactly one match (this block).
- ✅ Bot health: PID 85470 still running at session-end with no restart; heartbeat fresh.
- ✅ Anchor integrity — README backlink to `CLAUDE-sessions.md#session-115--substrate-maturity-review-7-forward-collecting-observations-re-read-2026-05-12-outcome-b--documentation-only` resolves to this heading.
