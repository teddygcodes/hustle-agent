...
- **Paper performance** (current — from `bot/state/paper_trades.json`, the ground-truth ledger, May 8, 2026):
  - `vig_stack`: 193 settled, **+$938.08**, 135W/7L/51EE (70% WR, +$4.86/trade) — **+$297 over the last 2 days on 8 settled trades**, post-Session-53 family caps appear to be working as intended. The "outlier-dependent" framing from Session 43-investigate has weakened materially; the ledger is now self-supporting, with one important Session 76 caveat: PAPER-DDF25C1E's +$296.92 was an already-fixed pre-Session-63 KXMLBGAME bug-pair recurrence, not a clean forward edge to project.
  - `live_momentum`: 96 settled, **+$10.20**, 26W/8L/62EE (27% WR, +$0.11/trade) — Win:Loss=0.261 structural finding from Session 40 still applies on the broader cohort, but post-Session-54 corrected-sizing data has now accumulated: **17 settled trades since May 5 net +$30.40 (+$1.79/trade, vs pre-fix −$0.26/trade pre-Session-54 baseline)**. Sample still thin (n=17 with 14 EE), but sign-flip is real evidence the Session 54 phantom-balance fix and Session 49 NBA/UFC sizing-down were both addressing real leak surfaces. May 15 / May 18 routine cluster is the first authoritative test.
  - **Net: +$948.28 across 289 settled trades** on the $10,500 paper base (~9.0% return). Bot is genuinely profitable from vig_stack as a structural arb (now ~$324 ahead of the May 7 snapshot on 16 new vig_stack settled trades) AND live_momentum has flipped from net-negative to net-positive on the post-Session-54 cohort.
...
17. [Recent Improvements (Apr 20–May 9)](#recent-improvements-apr-20may-9)
...
## Recent Improvements (Apr 20–May 9)

Sixteen arcs covering Apr 20 → May 9, then a session-by-session recap. **Full detail for each session lives in [CLAUDE-sessions.md](CLAUDE-sessions.md)**; the entries below link directly to the canonical session ☑ blocks.
