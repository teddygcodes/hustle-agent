...
- **May 7 — `heartbeat_stale` threshold split: 90s WARN / 180s CRITICAL (Session 78).** Tyler's "how is it critical but a false alarm?" pushback caught a real bug: `glint_status.py` flagged `heartbeat_stale` CRITICAL at any age > 90s, but the bot's `_heartbeat_loop` fires every 30s — 90s is only 3× the cycle, so normal asyncio event-loop jitter routinely tripped CRITICAL on a healthy bot. Verified live: snapshot at 23:29 ET flagged CRITICAL while heartbeat self-recovered to 28s age within 2 minutes. Surgical 12-LOC fix in [tools/glint_status.py:545-571](tools/glint_status.py:545): split into WARN at 90s (one missed cycle, watch) and CRITICAL at 180s (≥5× cycle = sustained gap = real wedge per Battle Scar #6 failure signatures). Post-fix verdict: "Degraded" with **0 CRITICAL** flags on the same bot state. Battle Scar #6 failure signatures still hit the 180s CRITICAL bar correctly because real wedges produce SUSTAINED stale heartbeats, not single-cycle blips. 7 new parametrized regression tests (10s/60s/89s no flag; 95s/150s WARN; 181s/600s CRITICAL). Test baseline: **1,526 passed / 0 failed / 0 skipped** (Session 77 baseline 1,519 + 7 new). No bot restart needed (glint_status read-only). Out of scope held: §3 health pulse stale-cache issue (separate code path, already on the queue).
