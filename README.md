...
- **Test baseline: 1,519 passed / 0 failed** (Session 77 added 1 regression test for the heartbeat-running=True self-correcting fix; Session 76 was doc-only and preserved the Session 75 baseline; Session 75 added 7 regression tests for the volume-spike live_momentum candidate and report diagnostics; Session 73 added 5 regression tests for the per-pair-key dedup methodology; Session 72 added 20 regression tests for `cross_market_correlation`; Session 71 added 4 regression tests for the persisted-cooldown init-read path; Session 70 closed the documented `test_paper_trades_open_count_matches_positions_active` live-state race via mtime-fence helper + 3 regression tests). **Zero baseline failures, zero skips, zero flakes** — Session 70's discipline rule in CLAUDE.md "Style Rules" section codifies this as the standing bar going forward. Battle Scars: **17 entries**, with **#15's operational rule retired post-Session-71** (the structural failure mode is still documented for posterity; the operational note now points forward — restart anytime). Session 57's calendar-vs-commit-timestamp pattern remains a discipline note pending a third recurrence to promote to Battle Scar #18. Discovery agent: **10 heuristics** unchanged post-Session-71. Strategy lab candidates: 3 (`example_total_points_under`, `cross_market_correlation`, `volume_spike_live_momentum`).
...
Phase 0 found Branch B, not Branch A: `live_ticks.jsonl` has **434,471 ticks across 9 sports / 773 tickers over 30d** but no per-tick traded-volume field; `universe.jsonl` has scan-level `volume`, `volume_24h`, and `open_interest`; `live_watcher` journals scan-level `total_vol` but `_log_tick` records price/game-state/DQS only. Built [tools/strategy_lab/candidates/volume_spike_live_momentum.py](hustle-agent/tools/strategy_lab/candidates/volume_spike_live_momentum.py), a coarse `volume_24h` proxy candidate with Session 51 tunables (`VOLUME_LOOKBACK_TICKS=12`, `VOLUME_BASELINE_TICKS=60`, `MIN_VOL_RATIO=1.5`) and Session 73 `pair_key=f"{ticker}|long"` dedup. Real 30d run: **14 emits / 4 unique pair-keys / 1 resolved / -81.00¢ mean CLV / -$81.00 P&L**, with single-sport NBA concentration. Sensitivity `{1.2,1.5,2.0,3.0}` is flat; 70/30 train/test has no resolved test pair-key; dip-only baseline beats it (**47 unique / 4 resolved / +27.5¢ / +$110**). Outcome C: overlay adds no value, strengthening the future live_momentum kill-session case. Lab infra now also renders cross-sport diagnostics and per-sport pair-key breakdowns. No `bot/` production code, no tick-volume instrumentation, no restart. Full suite **1,518 passed / 0 failed / 0 skipped**.

### ☑ Session 76 — `outlier_pnl` PAPER-DDF25C1E investigation: Outcome B ship shape, already-fixed bug-pair (May 8, doc-only)
§10 flagged `PAPER-DDF25C1E` as a NOTABLE `outlier_pnl`: +$296.92 on `KXMLBGAME-26MAY061905TEXNYY-NYY`, 43.25% of the `vig_stack/mlb` absolute-P&L cohort (`$686.50`). Phase 0 pulled the trade (`type=vig_stack`, `side=no`, `571` contracts, `entry_price=0.35`, `exit_price=0.87`, `exit_reason=auto_take_profit`, resolved `2026-05-07T00:12:29Z` / May 6 20:12 ET) and then found the archived decision in `decisions-2026-05-04.jsonl.gz`: `opp_type=vig_stack_futures`, all gates passed, `contracts=571`, `cost_dollars=199.85`. That timestamp is before the Session 71 restart/deploy boundary (`2026-05-08T00:29Z`), so this trade ran on the old SFPHI-shaped path. Pattern check: pre-restart KXMLBGAME decisions were `vig_stack_futures` (7 accepts / 13 rejects); post-restart KXMLBGAME decisions are `vig_stack_series` (1 accept / 1 reject), and the post-fix accept sized at `$49.98`. Settled pre-fix KXMLBGAME rows include both winners (+$296.92 DDF, +$172.52 SFPHI) and losers (-$199.88, -$6.46, -$5.95), so this is not a clean recurring edge. **Decision:** no production code, no tests, no restart. Outcome A rejected because Sessions 62/63 already fixed the live path; Outcome C rejected because this is bug-shaped rather than strategy-shaped; D rejected because the mechanism is identifiable. Watch-list trigger reopens only on post-`2026-05-08T00:29Z` KXMLBGAME futures labels, post-restart KXMLBGAME auto-exits before settlement, or >=3 post-restart outliers in `vig_stack/mlb` after excluding historical pre-Session-63 rows. Full suite remains **1,518 passed / 0 failed / 0 skipped**.

### ☑ Session 80 — `snapshot_universe` deadline-clip already-known structural state, hypothesis contradicted (May 8, doc-only Pattern C)
Investigation of the constant deadline-clip WARN (683-926 pages flushed per scan since at least 2026-05-07 14:34 ET) and overnight cadence collapse (two ~4h gaps 00:03 → 04:22 → 08:45 ET, `scans_today=2`). Phase 0 ran three independent measurements before locking scope, per the prior-session "verify premise" discipline. **Truncation:** ~30-48% of universe missing per scan (1,600-1,900 captured vs ~3,600 estimated complete), 683-926 pages flushed. **No clean baseline exists** in either log; earliest deadline-clipped scan is **2026-05-05 19:53:48 ET**, and CLAUDE.md's existing Apr 30 Session 28-followup re-verification record (cursor_rows median 1949 across n=32 scans, partial flag still 100%) confirms the pattern is documented expected state under current Kalshi load — at least 3 days old, plausibly back to Apr 26. **Truncated tail is mainstream NBA/NHL** markets (alphabetical cursor order cuts late-alphabet families), NOT filtered KXATPCHALLENGER-style noise. **Decisions are flowing** (161 on 2026-05-08 across KXINX/KXMLB/KXNHL/KXNBA/KXHIGH*) with 32 KXNBA + KXNHL decisions confirming we are still attribution-capable on the truncated tail. **Cadence diagnosis contradicts the hypothesis:** Gap 1 (00:03 → 04:22 ET) was NOT a gap — scan_id 20260508T044907 ran 3.5h on 6 transient `Connection reset by peer` retries; live_watcher emitted ticks every ~10-11s throughout, Battle Scar #13's `run_in_executor` wrap (Session 39) preserved event-loop responsiveness. Gap 2 (04:22 → 08:26 ET) was a real 4h idle but caused by a `MORNING: weather-only scan` route at 08:26:18 ET overriding the IDLE-30min schedule that fired at 04:33:57 ET — a `bot/scheduler.py` design issue, not deadline-clip compounding. **Architecture review (`bot/universe.py:50-200`):** `_SNAPSHOT_DEADLINE_SEC = 300` is a hardcoded constant whose own 30-line evidence comment explicitly says *"not yet another bump of this constant"* — Session 28-2 (per-series-paginated rewrite) is the structural fix, explicitly deferred unless specific triggers fire (cohort_report bias, partial-rate WARN order-of-magnitude regression, cursor_rows worsening). None of those triggers fire today. **Decision: Outcome C — Pattern C ship.** Hypothesis contradicted, cure explicitly deferred, decisions flowing, bot profitable (+$247 overnight from `vig_stack`). No production change, no test change, no bot restart. Watch-list trigger captures cursor_rows < 1500 sustained, pages flushed < 600 sustained, decisions volume < 50/day for 3+ days coincident with cursor_rows degradation, cadence collapse with NEW mechanism (not Kalshi retry-storm or morning-routine), or cohort_report >50% bias on active-strategy series. Out-of-scope follow-ups documented for separate sessions if pursued: morning-routine scheduling override (Gap 2's actual cause, `bot/scheduler.py`), Kalshi API retry-storm sensitivity (Gap 1's actual cause). Mirrors Pattern C precedents 18.5/38a-2/40/41/42/67-investigate/68/69/74. The session brief's "verify premise before scope" discipline paid for itself again. **Note (Session 81 follow-up):** the "MORNING override" Gap 2 attribution was imprecise — Session 81 verified that MORNING is a 9-second briefing helper at `bot/scanner.py:744-752`, not a 4h override; the actual 4h gap on 5/8 (04:33 → 08:45 ET, 7 missed IDLE-30min cycles) remains unidentified. See Session 81 entry below.

### ☑ Session 81 — alleged MORNING weather-only override hypothesis contradicted (May 8, doc-only Pattern C)
Session 80 deferred follow-up on the alleged Gap 2 cause. Phase 0 located the routine in code and **contradicted the override hypothesis.** The `MORNING: weather-only scan` log line is emitted at [bot/scanner.py:749](hustle-agent/bot/scanner.py:749) inside `morning_weather_scan()` — a **9-line function** that calls `scan_weather_markets()` and returns the list (docstring: *"Lightweight scan that only checks weather markets. Called by the morning briefing — no sports scan, no API usage."*). Caller is `_send_morning_briefing()` at `bot/scheduler.py:282-345`, gated at `bot/scheduler.py:64-67` by `MORNING_BRIEFING_HOUR <= current_hour < MORNING_BRIEFING_CUTOFF_HOUR and state.get("last_morning_briefing") != today_str` (constants `MORNING_BRIEFING_HOUR = 8`, `MORNING_BRIEFING_CUTOFF_HOUR = 20`, ET-aware). It is invoked from `check_scheduled_events(bot)` at `bot/main.py:1225` once per main-loop iteration. **Override vs additive: NEITHER** — MORNING is a regular function call inside the briefing, returning control to the main loop after ~9 seconds; does not touch `scan_interval`, does not reset the IDLE timer, does not preempt SCAN CYCLE. After correcting a timezone misread (log timestamp brackets are **ET**, not UTC — verified by `[2026-05-07 20:09:01]` bracketing payload `SCAN CYCLE — 2026-05-08 00:09:01 UTC`), MORNING fires once per day at 08:02-09:41 ET (variability is when the main loop hits the briefing gate after restart) and runs **8-17 seconds end-to-end**; this cannot cause a 4-hour gap. **Cost of the 5/8 gap: $0 direct.** Decisions in gap window: 78 entries (2 accept, 76 reject), entirely from `live_watcher` paths; comparable non-gap 4h window today: 181 decisions (5 accept, 176 reject) — acceptance rate near-identical (2.6% vs 2.8%), raw count is 43% of baseline because SCAN CYCLE-driven decisions are absent. Paper trades entered during/after the gap on 5/8: **0**. MORNING found 0 weather opportunities. **Decision: Outcome C — Pattern C ship, hypothesis contradicted.** No code change, no test change, no bot restart. Watch-list update **supersedes Session 80's "morning-routine scheduling overrides (Gap 2 shape)" sub-clause**: a 4h+ overnight gap that is NEITHER a Kalshi-retry-storm scan in progress NOR coincident with a ≤20s briefing/MORNING block points to a real `_main_loop` scan-interval regression worth a focused session. The actual cause of the 4h gap on 5/8 (04:33 → 08:45 ET, 7 missed IDLE-30min cycles) remains unidentified and is explicitly out of scope per the Session 81 brief. Mirrors Pattern C precedents 18.5/38a-2/40/41/42/67-investigate/68/69/74/80 — **Pattern C count now 11.** Lesson: always cross-check log timezones before drawing time-of-day conclusions; a 4h misread reads as "MORNING fires pre-dawn" when reality is "MORNING fires at 8am as designed."
