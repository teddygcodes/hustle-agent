...
- **Session 1** (Apr 20, 2026) — Settlement + pattern pipeline rebuild — wired exited_early settlements into audit (was missing 58 of 93 paper resolutions). [→ details](CLAUDE.md#session-1--settlement--pattern-pipeline-apr-20)
- **Session 2** (Apr 20, 2026) — Active strategy retuning — vig_stack volatile floor 0.90→0.93, MOMENTUM_LEADER_MIN bump+revert, MOMENTUM_DISABLED_SPORTS adds tennis (later refined). [→ details](CLAUDE.md#session-2--active-strategy-retuning-apr-20)
- **Session 3** (Apr 23, 2026) — Live-watcher ESPN restoration — User-Agent + certifi SSL + structured error logging fixed silent ESPN fetch failures. [→ details](CLAUDE.md#session-3--live-watcher-espn-restoration-apr-23)
- **Session 4** (Apr 23, 2026) — Scheduler + bot_state revival — fixed hour-gate `==`→`>=`, persisted total_pnl, daily-rollover zero crypto counter, startup drift warnings. [→ details](CLAUDE.md#session-4--scheduler--bot_state-revival-apr-23)
- **Session 5** (Apr 23, 2026) — State hygiene — clv filter to active strategies, live_ticks daily rotation, lock heartbeat per scan, 108MB→~10MB. [→ details](CLAUDE.md#session-5--state-hygiene-apr-23)
- **Session 6** (Apr 24, 2026) — Closed-loop data collection — decisions.jsonl audit log per accept/reject + counterfactual CLV records + cohort_report tool. [→ details](CLAUDE.md#session-6--closed-loop-data-collection-apr-24)
- **Session 7** (Apr 24, 2026) — Decision-log observability gaps — wp_edge as live-momentum edge proxy + 30s heartbeat lock-touch. [→ details](CLAUDE.md#session-7--decision-log-observability-gaps-apr-24-planned)
- **Session 8** (Apr 24, 2026) — Stratified CF sampling — per-gate top-K instead of global top-K (was systematically excluding edge_below_threshold rejects). [→ details](CLAUDE.md#session-8--stratified-cf-sampling-apr-24-shipped)
- **Session 9** (Apr 24, 2026) — MFE/MAE per position — Session 9 ratchet on observed bid; settlement-time extension at clv.py. [→ details](CLAUDE.md#session-9--mfemae-per-position-apr-24-shipped)
- **Session 10** (Apr 24, 2026) — Gate-context enrichment in `extra` — distance-from-threshold per gate (forecast_in_bucket, edge_below_threshold, low_liquidity, etc.). [→ details](CLAUDE.md#session-10--gate-context-enrichment-in-extra-apr-24-done)
- **Session 11** (Apr 25, 2026) — Fair-value calibration loop — predictions.jsonl + tools/calibration_report.py with Brier scores per opp_type. [→ details](CLAUDE.md#session-11--fair-value-calibration-loop-apr-25-done)
- **Session 12** (Apr 25, 2026) — Universe log — every active Kalshi market per scan with `scanned_by` attribution; first snapshot found 53% of markets ignored. [→ details](CLAUDE.md#session-12--universe-log-apr-25-shipped)
- **Session 13** (Apr 25, 2026) — Hypothetical strategy framework — Strategy Protocol + offline back-tester sharing compute_clv_cents + parameter sweep tool (3 sub-sessions). [→ details](CLAUDE.md#session-13--hypothetical-strategy-framework-apr-25-shipped-3-sub-sessions)
- **Session 14** (Apr 25, 2026) — Regime tags — 4-axis regime tagger (time_of_day, day_of_week, sport_phase, event_horizon_hr) on every record + backfill script. [→ details](CLAUDE.md#session-14--regime-tags-apr-25-shipped)
- **Session 15** (Apr 25, 2026) — Live order microstructure — bot/order_microstructure.py plumbing-only; verification deferred until PAPER_MODE=False. [→ details](CLAUDE.md#session-15--live-order-microstructure-apr-25-shipped-plumbing-only--verification-deferred-until-paper_modefalse)
- **Session 15.5** (Apr 25, 2026) — Data integrity hardening — heartbeat dual-update, conftest logger isolation, partial-rate metering, close_ts threading. [→ details](CLAUDE.md#session-155--data-integrity-hardening-before-7-day-unattended-run-apr-25-shipped)
- **Session 16** (Apr 25, 2026) — Excursion gap-math fix — settlement-time MFE extension at clv.py:check_clv_settlements; gap is now ≥0¢ on every record. [→ details](CLAUDE.md#session-16--excursion-gap-math-fix-apr-25-shipped-deeper-fix-scope)
- **Session 17** (Apr 26, 2026) — Tracker cadence audit — Outcome B: 30s `_position_check_loop` added so 90% of live_momentum positions stop being unobserved. [→ details](CLAUDE.md#session-17--tracker-cadence-audit-for-live_momentum-apr-26-shipped-outcome-b)
- **Session 18** (Apr 26, 2026) — live_journal.json analysis tool — 3 findings: TRAILING_STOP/DOLLAR_STOP fire 0%, UFC mechanically different, 56-91% watch-but-no-enter. [→ details](CLAUDE.md#session-18--live_journaljson-analysis-tool-apr-26-shipped)
- **Session 19a-peakfix** (Apr 26, 2026) — Production peak-tracking bug fix — `bot/live_watcher.py:2225` setdefault chicken-and-egg; TRAILING_STOP physically could not fire prior to fix. [→ details](CLAUDE.md#session-19a-peakfix--production-peak-tracking-bug-fix-apr-26-shipped)
- **Session 19** (Apr 26, 2026) — Tick-replay back-tester for live_momentum — TickStrategy Protocol + back-tester + 15-variant sweep shipping MOMENTUM_LEADER_MIN: 0.70→0.65 (3 sub-sessions: 19a/19a-followup/19b/19c). [→ details](CLAUDE.md#session-19--tick-replay-back-tester-for-live_momentum-apr-2627-complete--3-sub-sessions-shipped)
- **Session 21** (Apr 27, 2026) — live_watcher skip_reason instrumentation — 11 gates wired into _journal_record_scan; closes Session 18 Finding #3 watch-but-no-enter visibility. [→ details](CLAUDE.md#session-21--live_watcher-skip_reason-instrumentation-apr-27-shipped)
- **Session 24** (Apr 28, 2026) — Weekly digest tool — tools/weekly_digest.py aggregator over 8 sections (later superseded by Session 35). [→ details](CLAUDE.md#session-24--weekly-digest-tool-apr-28-shipped)
- **Session 28** (Apr 28, 2026) — partial_snapshots tuning — Outcome C mixed: bumped _SNAPSHOT_DEADLINE_SEC 90→180→300 (followup); transient retry loop on connection-reset/timeout. [→ details](CLAUDE.md#session-28--partial_snapshots-tuning-apr-28-shipped)
- **Session 29** (Apr 28, 2026) — Live journal write regression — Session 21 write rate jump corrupted live_journal.json; recovered via raw_decode fallback in `_journal_append`. [→ details](CLAUDE.md#session-29--live-journal-write-regression-apr-28-shipped)
- **Session 29-followup** (Apr 28, 2026) — bet/exit/session_end journal write regression investigation — no fix needed; per-event-type regression tests added to close verification gap. [→ details](CLAUDE.md#session-29-followup--betexitsession_end-journal-write-regression-apr-28-shipped-investigation-no-fix-needed)
- **Session 30** (Apr 28, 2026) — live_momentum research dataset + bucket analysis — tools/live_momentum_dataset.py + tools/live_momentum_buckets.py with leakage property test. [→ details](CLAUDE.md#session-30--live_momentum-research-dataset--bucket-analysis-apr-28-ship-now-shipped-apr-28-29)
- **Session 30-followup** (Apr 29, 2026) — Challenger-circuit edge probe — Outcome C HOLD: settled subset 100% yes_won (severity bias); does not justify re-enable. [→ details](CLAUDE.md#session-30-followup--challenger-circuit-edge-probe-apr-29-30-min-no-config-change)
- **Session 30-followup-2** (Apr 29, 2026) — Dataset 84%-missing-CFs fix — CF-only fallback rows when no journal+ticks; dataset 89→413 rows. [→ details](CLAUDE.md#session-30-followup-2--dataset-84-missing-cfs-fix-apr-29-shipped-30min-investigation--small-fix)
- **Session 33** (Apr 29, 2026) — DQS persistence to live_ticks rows — 5 LOC in `_tick_momentum` to write `dqs_for_log` to tick payload (forward-only). [→ details](CLAUDE.md#session-33--dqs-persistence-to-live_ticks-rows-apr-29-shipped--commit-e479178)
- **Session 34** (Apr 29, 2026) — match_phase regime axis for tennis/UFC/IPL — 5th regime axis added; 6 writers + backfill of 114,772 records. [→ details](CLAUDE.md#session-34--match_phase-regime-axis-for-tennis--ufc--ipl-apr-29-shipped)
- **Session 35** (Apr 29, 2026) — Daily + weekly report generators — tools/daily_report.py (3 AM ET) + tools/weekly_report.py (Sun 6 AM ET); retired weekly_digest.py. [→ details](CLAUDE.md#session-35--daily--weekly-report-generators-apr-29-shipped)
- **Session 37** (Apr 29, 2026) — Test hygiene cleanup — 10 documented pre-existing test failures cleaned up via group-paid-down approach; baseline 0 failures. [→ details](CLAUDE.md#session-37--test-hygiene-cleanup-apr-29-shipped)
- **Session 38a** (Apr 29, 2026) — Outcome A: ATP main-tour disable re-evaluation — re-enabled atp main tour (n=56 settled CFs +11.32¢, 4 historical paper trades 3W/1L +$8.60). [→ details](CLAUDE.md#session-38a--atp-main-tour-disable-re-evaluation-apr-29-shipped--outcome-a)
- **Session 38a-2** (Apr 30, 2026) — Outcome B: WTA main-tour disable re-evaluation — kept WTA disabled (negative EV despite 71% +CLV: avg win +31¢ vs avg loss −80¢). [→ details](CLAUDE.md#session-38a-2--wta-main-tour-disable-re-evaluation-apr-30-shipped--outcome-b-doc-only)
- **Session 39** (Apr 30, 2026) — CRITICAL: asyncio event-loop blocking on snapshot_universe — wrapped in `loop.run_in_executor`; closed 12-hour bot wedge incident. [→ details](CLAUDE.md#session-39--fix-asyncio-event-loop-blocking-on-snapshot_universe-apr-30-shipped-critical)
- **Session 40** (Apr 30, 2026) — Outcome C: live_momentum EE-rate investigation — exits balanced (Pattern C); structural Win:Loss=0.261 finding. [→ details](CLAUDE.md#session-40--live_momentum-ee-rate-investigation-apr-30-shipped--outcome-c-doc-only)
- **Session 41** (Apr 30, 2026) — Outcome C: TP/SL ratio sweep for live_momentum — SL axis structurally flat across all 4 SL values; ratio shape not the leak. [→ details](CLAUDE.md#session-41--tpsl-ratio-sweep-for-live_momentum-apr-30-shipped--outcome-c-doc-only--phase-0-plumbing)
- **Session 42** (Apr 30, 2026) — Outcome C: per-sport TP/SL variants for live_momentum — Pattern C across NBA/NHL/UFC; SL-axis-flat reproduces per-sport. [→ details](CLAUDE.md#session-42--per-sport-tpsl-variants-for-live_momentum-apr-30-shipped--pattern-c-across-all-sports-doc-only--phase-2-plumbing)
- **Session 43a** (May 1, 2026) — Discovery Agent framework + 2 SFPHI-catching heuristics — outlier_pnl + cohort_emergence + SFPHI regression test (P0). [→ details](CLAUDE.md#session-43a--discovery-agent-framework--2-sfphi-catching-heuristics-may-1-shipped--first-real-run-produced-unexpected-lead)
- **Session 43-investigate** (May 1, 2026) — vig_stack_futures cross-sport lead — SFPHI singleton confirmed; surfaced separate `non_stable_below_weather_floor`-on-futures lead. [→ details](CLAUDE.md#session-43-investigate--vig_stack_futures-cross-sport-lead-may-1-doc-only-follow-up-to-session-43a-finding)
- **Session 43b** (May 1, 2026) — 6 additional discovery heuristics — threshold_proximity, counterfactual_hotspots, universe_gap, live_tick_anomalies, cadence_outcome, log_error_spike + cohort_emergence refinement. [→ details](CLAUDE.md#session-43b--discovery-agent-6-additional-heuristics--cohort_emergence-refinement-may-1-shipped--3-high-counterfactual-findings-on-day-2-run)
- **Session 44** (May 1, 2026) — Discovery agent housekeeping — gate-flow analysis disambiguated WTA `no_leader` finding; IPL spot-check too thin. [→ details](CLAUDE.md#session-44--discovery-agent-housekeeping-gate-flow-analysis--ipl-spot-check--lead-reprioritization-may-1-doc-only-30min)
- **Session 45** (May 1, 2026) — Outcome C: `no_vol_growth_first_seen` retuning HELD — gate is binary cycle-delay not a tunable threshold. [→ details](CLAUDE.md#session-45--no_vol_growth_first_seen-retuning-held-may-1-doc-only-outcome-c)
- **Session 46** (May 1, 2026) — Outcome C: `no_vol_growth_idle` retuning HELD — gate IS tunable but cross-cohort mean −1.34¢; top 2 positive cohorts disabled downstream. [→ details](CLAUDE.md#session-46--no_vol_growth_idle-retuning-held-may-1-doc-only-outcome-c)
- **Session 47** (May 1, 2026) — `counterfactual_hotspots` cross-cohort context refinement — severity demotion ladder for cherry-picked findings. [→ details](CLAUDE.md#session-47--counterfactual_hotspots-cross-cohort-context-refinement-may-1-1h-coder-discovery-agent-only)
- **Session 48** (May 1, 2026) — `concurrent_attack_angles` 9th heuristic — surfaces multi-strategy-per-event candidates from data already collected. [→ details](CLAUDE.md#session-48--concurrent_attack_angles-heuristic-may-1-25h-coder-discovery-agent-only--9th-heuristic-shipped)
- **Session 49** (May 1, 2026) — Per-sport size_multiplier for live_momentum — NBA + UFC sized down 50%; first production-code change of the day. [→ details](CLAUDE.md#session-49--per-sport-size_multiplier-for-live_momentum-nba--ufc-sized-down-50-may-1-15h-coder-first-production-code-change-today)
- **Session 50** (May 1, 2026) — Trade-record observability: confidence + dqs + sport on live_momentum paper_trades.json (forward-only). [→ details](CLAUDE.md#session-50--trade-record-observability-confidence--dqs--sport-on-live_momentum-may-1--may-2-transition-1h-coder)
- **Session 51** (May 2, 2026) — Strategy Lab v1 — tools/strategy_lab/ for rapid hypothesis prototyping; days/weeks → seconds for "is this idea worth pursuing?". [→ details](CLAUDE.md#session-51--strategy-lab-v1-rapid-hypothesis-prototyping-for-new-strategies-may-2-early-hours-25h-coder-no-production-code)
- **Session 52** (May 3, 2026) — Telegram rate-limit hardening — 429 backoff + edit throttle + dedup + state surfacing; Battle Scar #15 codified (later retired by Session 71). [→ details](CLAUDE.md#session-52--telegram-rate-limit-hardening-429-backoff--edit-throttle--dedup--state-surfacing-may-3-25h-coder-restart-deferred-per-battle-scar-15)
- **Session 53** (May 4, 2026) — Per-family vig_stack max_position_dollars cap — KXINX/KXMLBGAME $50, KXHIGH* $150, healthy $200. [→ details](CLAUDE.md#session-53--per-family-max_position_dollars-cap-for-vig_stack-kxinxkxmlbgame-50-kxhigh-150-healthy-200-may-4-1h-coder-restart-gated-on-telegram-cool-down-per-battle-scar-15)
- **Session 54** (May 5, 2026) — live_watcher correctness pass — paper balance fix (was $500+pnl, now PAPER_STARTING_BALANCE+pnl), exit_reason persistence, ScoreSnapshot period. [→ details](CLAUDE.md#session-54--live_watcher-correctness-pass-paper-balance-exit_reason-scoresnapshot-period-may-5-2h-coder)
- **Session 55** (May 6, 2026) — `settlement_vs_rationale` 10th discovery heuristic — KXHIGHMIA founding regression P0; Glint Analyst LLM proposal deferred. [→ details](CLAUDE.md#session-55--settlement_vs_rationale-10th-discovery-heuristic-kxhighmia-founding-regression-p0-may-6-25h-coder-doc--tools-only--no-bot-touch-no-restart)
- **Session 56** (May 6, 2026) — Disable sports_arb strategies + remove stale LIVE_TRAILING_STOP reference — 4-layer defense-in-depth. [→ details](CLAUDE.md#session-56--disable-sports_arb-strategies--remove-stale-live_trailing_stop-reference-may-6-45-min-4-layer-defense-in-depth--dead-code-cleanup)
- **Session 56.5** (May 6, 2026) — Restore SFPHI regression test — fixed 3-session calendar drift via relative-date fixture anchoring. [→ details](CLAUDE.md#session-565--restore-sfphi-regression-test-may-6-30min-doc--test-only--no-bot-touch-no-restart)
- **Session 57** (May 6, 2026) — Disabled-sport "leak" was heuristic precision drift, not a bot bug — tightened MOMENTUM_DISABLED_SINCE cutoffs to commit timestamp. [→ details](CLAUDE.md#session-57--disabled-sport-leak-was-heuristic-precision-drift-not-a-bot-bug-may-6-15h-tools--tests-only--no-bot-touch-no-restart)
- **Session 58** (May 7, 2026) — Notifier `HTTPXRequest is not initialized` failures — Outcome D5: watcher asyncio tasks must be cancelled before notifier teardown (Battle Scar #17). [→ details](CLAUDE.md#session-58--notifier-httpxrequest-is-not-initialized-failures-not-a-startup-race-watcher-asyncio-tasks-tick-on-dead-notifier-post-stop-may-7-3h-fix-in-botmainpyglintbotstop)
- **Session 58.5** (May 7, 2026) — Defense-in-depth: `_stopping` flag short-circuits notifier-shutdown retry log spam; closes 2 residual errors per restart. [→ details](CLAUDE.md#session-585--_stopping-flag-short-circuits-notifier-shutdown-retry-log-spam-may-7-45min-defense-in-depth--log-cleanup)
- **Session 59** (May 7, 2026) — Planner-tuned tools/glint_status.py consolidator — read-only on-demand status snapshot with diff + watch-list evaluation. [→ details](CLAUDE.md#session-59--planner-tuned-toolsglint_statuspy-consolidator-may-7-2h-read-only-planner-workflow-tool)
- **Session 60** (May 7, 2026) — Daily-report launchd cron + paper-ledger consistency repair — installed com.hustle-agent.daily-report, fixed paper-trades open/resting state lifecycle. [→ details](CLAUDE.md#session-60--daily-report-launchd-regression--paper-ledger-consistency-fix-may-7-investigation-then-fix-operationaldata-integrity)
- **Session 61** (May 7, 2026) — Challenger CF re-evaluation HELD + live_momentum zero-entry diagnosis passive — disable holds at n=398/leader-loss=122. [→ details](CLAUDE.md#session-61--challenger-cf-re-evaluation-held--live_momentum-zero-entry-flag-diagnosed-passive-may-7-investigation-only)
- **Session 62** (May 7, 2026) — Outcome A: KXMLBGAME family-cap D6 fix for `vig_stack_futures` sizing — extended sizing tuple to include vig_stack_futures. [→ details](CLAUDE.md#session-62--kxmlbgame-family-cap-d6-fix-for-vig_stack_futures-sizing-may-7-urgent-open-position-breach)
- **Session 63** (May 7, 2026) — KXMLBGAME per-game scanner classification fix — KX*GAME prefixes emit `vig_stack_series` (was incorrectly `vig_stack_futures`). [→ details](CLAUDE.md#session-63--kxmlbgame-per-game-scanner-classification-fix-may-7-structural-follow-up-to-session-62)
- **Session 64** (May 7, 2026) — Outcome B: WTA per-sport `MOMENTUM_LEADER_MIN` re-evaluation — shipped lever (`MOMENTUM_LEADER_MIN_PER_SPORT` dict) but kept WTA disabled. [→ details](CLAUDE.md#session-64--wta-per-sport-momentum_leader_min-re-evaluation-may-7-pattern-b--architectural-lever-shipped-wta-stays-disabled)
- **Session 65** (May 7, 2026) — Housekeeping bundle: 3 small fixes — Session 30-followup watch-list threshold raised, glint_status NEW counter consistency, removed legacy `bot/tools/`. [→ details](CLAUDE.md#session-65--housekeeping-bundle-3-small-fixes-may-7-30-min-no-production-code-touched)
- **Session 66** (May 7, 2026) — Strategy Candidates safety net in daily_report + glint_status — §10 surface for 7 strategy-relevant heuristics. [→ details](CLAUDE.md#session-66--strategy-candidates-safety-net-in-daily_report--glint_status-may-7)
- **Session 67** (May 7, 2026) — Outcome B: settlement_vs_rationale family-tail counter refinement + per-family floor override surface for vig_stack. [→ details](CLAUDE.md#session-67--settlement_vs_rationale-family-tail-counter-refinement--no_price_below_floor-phase-1-may-7-2h-outcome-b-per-family-floor-override-surface)
- **Session 68** (May 7, 2026) — Outcome C: `edge_below_threshold/nhl_futures` Phase-1 HELD — threshold sweep structurally flat; mechanism is fair-value-model error. [→ details](CLAUDE.md#session-68--edge_below_thresholdnhl_futures-phase-1-held-may-7-doc-only-outcome-c)
- **Session 69** (May 7, 2026) — Outcome C: `vig_stack_series.evaluate` fair-value formula calibration HELD — single sport (NHL n_effective=3); weather regression FALSIFIED. [→ details](CLAUDE.md#session-69--vig_stack_seriesevaluate-fair-value-formula-calibration-held-may-7-doc-only-outcome-c)
- **Session 71** (May 7, 2026) — TelegramNotifier reads persisted cooldown on init — closes Battle Scar #15 gap (~22 LOC); restart anytime, cooldown or not. [→ details](CLAUDE.md#session-71--telegramnotifier-reads-persisted-cooldown-on-init-may-7-30min-closes-battle-scar-15-gap)
- **Session 72** (May 8, 2026) — Outcome C: cross-market correlation Phase 0 + strategy_lab prototype — series-state asymmetry, not mispricing; founded scan-cadence amplification methodology lesson. [→ details](CLAUDE.md#session-72--cross-market-correlation-phase-0--strategy_lab-prototype-outcome-c-may-8-2h-no-production-code-lab-only)
- **Session 73** (May 8, 2026) — Strategy lab per-pair-key dedup fix — codified as durable lab infrastructure; future stateful candidates ship correct out of the box. [→ details](CLAUDE.md#session-73--strategy-lab-per-pair-key-dedup-fix-may-8-15h-lab-only-methodology-codified)
- **Session 74** (May 8, 2026) — Outcome C: `no_vol_growth_first_seen/nhl_game` HOLD — clean §10 HIGH; cycle-delay-disqualified per Session 45. [→ details](CLAUDE.md#session-74--no_vol_growth_first_seennhl_game-outcome-c-hold-may-8-doc-only-clean-10-high)
- **Session 75** (May 8, 2026) — Outcome C: volume-spike live_momentum overlay prototype — overlay is worse than dip-only baseline; flat sensitivity sweep. [→ details](CLAUDE.md#session-75--volume-spike-live_momentum-overlay-phase-0--strategy_lab-prototype-outcome-c-may-8-lab-only)
- **Session 76** (May 8, 2026) — Outcome B doc-only: `outlier_pnl` PAPER-DDF25C1E investigation — already-fixed pre-Session-63 KXMLBGAME bug-pair recurrence. [→ details](CLAUDE.md#session-76--outlier_pnl-paper-ddf25c1e-investigation-outcome-b-ship-shape-already-fixed-bug-pair-may-8-doc-only)
- **Session 77** (May 7, 2026) — `bot_state.json:running` self-correcting heartbeat fix — surgical 3-line fix; semantic becomes "running=True iff heartbeat loop is alive". [→ details](CLAUDE.md#session-77--bot_statejsonrunning-self-correcting-heartbeat-fix-may-7-surgical-3-line-fix)
- **Session 80** (May 8, 2026) — Outcome C: `snapshot_universe` deadline-clip already-known structural state — hypothesis contradicted (deadline-clip is pre-existing). [→ details](CLAUDE.md#session-80--snapshot_universe-deadline-clip-already-known-structural-state-hypothesis-contradicted-may-8-doc-only-pattern-c-no-production-change)
- **Session 81** (May 8, 2026) — Outcome C: alleged MORNING weather-only override hypothesis contradicted — MORNING is a 9-second briefing helper, not a 4h scheduler. [→ details](CLAUDE.md#session-81--alleged-morning-weather-only-override-hypothesis-contradicted-may-8-doc-only-pattern-c-no-production-change)
- **Session 82** (May 8, 2026) — Outcome C: 4h IDLE-30min cycle gap diagnosed as host-side macOS sleep — wall-clock sleep mechanism handles this by design. [→ details](CLAUDE.md#session-82--4h-idle-30min-cycle-gap-diagnosed-as-host-side-macos-sleep-may-8-doc-only-pattern-c-no-production-change)
- **Session 83** (May 8, 2026) — Outcome A: stale-data hygiene ship — vestigial `bot_state.total_pnl` removed + §3 always-shows-timestamp + Bob ride-along note. [→ details](CLAUDE.md#session-83--stale-data-hygiene-ship-vestigial-bot_statetotal_pnl-removed--3-always-shows-timestamp--bob-ride-along-may-8-outcome-a-three-fix-bundle)
- **Session 84** (May 8, 2026) — Strategy Candidate Promotion Bar protocol shipped — codified maturity bar for §10 candidates per heuristic + disqualifier checks. [→ details](CLAUDE.md#session-84--strategy-candidate-promotion-bar-protocol-shipped-may-8-doc-only-outcome-c-codified-maturity-bar-for-10)
- **Session 85** (May 8, 2026) — Outcome C: `no_vol_growth_first_seen/nhl_game` cycle-delay unblock investigation HELD — extended watch-list trigger to require sizing/orderbook instrumentation. [→ details](CLAUDE.md#session-85--no_vol_growth_first_seennhl_game-cycle-delay-unblock-investigation-outcome-c-hold-with-extended-watch-list-trigger-may-8-doc-only)
- **Session 86** (May 8, 2026) — Outcome A: live_momentum CF per-day dedup at emission — `(ticker, opp_type, sport, skip_reason, day)` semantic key in clv.py. [→ details](CLAUDE.md#session-86--live_momentum-cf-per-day-dedup-at-emission-may-8-outcome-a-scoped-clvpy-fix--3-tests)
- **Session 87** (May 8, 2026) — Outcome C: bar interpretation during 30-day deduped-CF transition HOLD until 2026-06-08 — natural convergence; zero false-positive operator cost. [→ details](CLAUDE.md#session-87--bar-interpretation-during-30-day-deduped-cf-transition-outcome-c-hold-until-2026-06-08-may-8-doc-only)
- **Session 88** (May 8, 2026) — Outcome A: README cleanup — 64 session paragraphs replaced with one-liners + Current Status refresh; 1,268 → 1,072 lines (15.4% reduction). [→ details](CLAUDE.md#session-88--readme-cleanup-64-session-paragraphs--one-liners--current-status-refresh-may-8-doc-only-scoped-readmemd-ship)
