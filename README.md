...
- **Paper performance** (current — from `bot/state/paper_trades.json`, the ground-truth ledger, May 6, 2026):
  - `vig_stack`: 169 settled, **+$317.41**, 114W/5L/50EE (67% WR, +$1.88/trade — outlier-dependent, see Session 43-investigate caveat)
  - `live_momentum`: 80 settled, **−$34.60**, 24W/8L/48EE (30% WR, −$0.43/trade) — Win:Loss=0.261 structural finding from Session 40; **sizing was at 9% of intended Kelly until Session 54 fixed the phantom-balance bug on May 5**, so all prior live_momentum P&L analysis is provisional pending 14d of corrected post-Session-54 data
  - **Net: +$282.81 across 249 settled trades.** Today (May 6): −$228.24 from 8 settlements, **two −$200 single-trade losses** (pre-Session-53 KXMLBGAME legacy + post-Session-53 KXHIGHMIA in a "healthy" family). Session 53 caps prevent the KXMLBGAME shape on future entries; the KXHIGHMIA loss surfaces a new structural-tail-risk lead worth investigating before May 14 day-14 routine.
...
- **Honest P&L position (May 6, post-Session-54):** Realized P&L sits at **+$282.81 across 249 settled trades** on the $10,500 paper base. Per-strategy: vig_stack **+$317.41** (67% WR, +$1.88/trade — outlier-dependent, would be ~−$130 without SFPHI + KXINX-81 + a few +$60 weather wins; rehabilitating via Filter F + Session 53 family caps); live_momentum **−$34.60** (30% WR, −$0.43/trade) with Win:Loss=0.261 asymmetric-loss multiplier — but at 9%-of-intended-scale sizing pre-Session-54. Today's (May 6) −$228 day driven by two pre-cap structural tail-risk losses; the cap regime is now in place but legacy oversize positions will continue to settle through ~May 8. **Bot is profitably trading vig_stack as a structural arb; live_momentum is ambiguous-EV pending corrected-sizing data.** The **May 14-18 routine cluster** (Session 53 KXINX day-14, Session 38a ATP re-validation, Session 49 NBA/UFC sizing, Session 22 MOMENTUM_LEADER_MIN) is the first sharp decision point at honest scale — and any `concurrent_fire_candidate` the agent surfaces between now and then that the lab validates becomes a NEW shippable strategy.
- **Test baseline: 1,390 passed / 0 failed** (was 1,359 at end-of-Session-51; +11 from Session 52 notifier hardening, +12 from Session 53 vig_stack family caps, +3 from Session 54 live_watcher correctness pass + the unrelated Session 52-followup gitignore audit). Battle Scars now: 16 entries (added #15 Telegram-429-state and #16 live_momentum-bankroll-must-be-configured).
- **Calendar of scheduled routines:** see `REPORT_CALENDAR.md` at the repo root. Daily 3 AM ET (daily report), daily 6 AM ET (discovery agent — 9 heuristics), weekly Sundays 6 AM ET (weekly report). One-offs still upcoming: **May 7** Session 39 day-7 flaky-Kalshi stress check; **May 12** weekly digest spot-check; **May 13** dual routine (Session 36 day-14 floor signal recheck + Session 38a ATP re-validation); **May 14** Session 53 KXINX day-14 P&L slope check; **May 15** Session 49 per-sport sizing re-validation (now on corrected-balance data per Session 54); **May 18** Session 22 MOMENTUM_LEADER_MIN re-validation (also corrected-balance). May 6 Session 36 day-7 routine has fired. Each routine writes its report file to disk before any interpretation work — partial chat death preserves data.
...
17. [Recent Improvements (Apr 20–May 6)](#recent-improvements-apr-20may-6)
...
## Recent Improvements (Apr 20–May 6)
