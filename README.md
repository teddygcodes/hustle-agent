...
- **Apr 29 hygiene + ATP re-enable (Sessions 36, 36-followup, 37, 38a).** Session 36 vig_stack TP/SL exemption (above) + exit_reason persistence; Session 36-followup fixed two Session-34 backfill_regime test oversights; **Session 37** cleaned up 10 documented pre-existing test failures via group-paid-down approach (Group A `__new__()` bypass, Group B stale config pins, Group C silenced watchdog dead-code removal, Group D per-day-cap monkeypatch); Session 38a re-enabled ATP main tour. **Test baseline: 1,165 passed / 0 failed** (was 12 documented pre-existing failures going into Apr 29 evening).
- **Apr 29+ Evidence-Driven Retuning Arc (Sessions 38a shipped; 38b–e planned).** Session 38a was the trigger session: an Apr 29 night re-run of `tools/live_momentum_buckets.py` against the regenerated dataset (714 rows, 321 with settled CLV) surfaced 3 real signals + 2 hidden infrastructure gaps. ATP main-tour re-enable shipped tonight; IPL sport-disable (n=25 settled at −35.88¢ avg CLV, 28% +CLV — Session 38b), `MOMENTUM_LEADER_MAX` ceiling investigation for >=90¢ leaders (n=22 settled at −28.23¢ avg CLV — Session 38c), match_phase dataset extractor wiring (Session 34 hidden gap — Session 38d), and bucket report n-column split for total-vs-settled (Session 38e) all queued for the next 24-72 hours per the discipline of "ship one, measure, then ship the next."
- **Calendar of scheduled routines:** see `REPORT_CALENDAR.md` at the repo root. Daily 3 AM ET, weekly Sundays 6 AM ET (both fully automated reports). One-offs: Apr 30 cursor stability re-check, **May 6 Session 36 day-7 hold-to-settlement check**, May 12 weekly digest spot-check, **May 13 dual routine** (Session 36 day-14 floor signal recheck + Session 38a ATP re-validation), May 18 MOMENTUM_LEADER_MIN re-validation. Each routine writes its report file to disk before any interpretation work — partial chat death preserves data.
...
15. [Recent Improvements (Apr 20–29)](#recent-improvements-apr-2029)
...
**Apr 29+ — Evidence-Driven Retuning Arc (Sessions 38a–e — 38a shipped, 38b/c/d/e queued).** Sessions 12–37 built the eyes (instrumentation) and the reactive loop (find what's broken → fix it). Session 38+ opens the *active interrogation* loop. **Investigation #1 (Apr 29 night)** re-ran `tools/live_momentum_buckets.py` against the regenerated dataset (714 rows, 321 with settled CLV) and surfaced 3 real signals + 2 hidden infrastructure gaps. ATP main-tour re-enable shipped tonight; IPL sport-disable (n=25 settled at −35.88¢, 28% +CLV — Session 38b), `MOMENTUM_LEADER_MAX` ceiling for >=90¢ leaders (n=22 at −28.23¢ — Session 38c), match_phase axis wiring into the dataset extractor (Session 38d hidden gap from Session 34), and bucket report n-column total-vs-settled split (Session 38e usability fix) all queued. Discipline: each session ships independently to preserve attribution per-change.
