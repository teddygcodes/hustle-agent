...
- **Apr 27–29 pre-checkpoint coverage arc (Sessions 21, 23, 24, 28, 29, 30).** Continuation of strategy iteration with focus on data completeness for the May 2 retuning checkpoint. Session 21 instrumented `live_watcher` with `skip_reason` capture across 11 gates (forward-only — closes Session 18 Finding #3's "watch-but-no-enter" black hole); Session 23 added `live_momentum` counterfactuals with stratified-by-(sport,skip_reason) sampling and per-day cap (mirrors Session 8 vig_stack pattern but for tick strategies; ~10×/day rate); Session 24 shipped `weekly_digest.py` aggregator over 8 sections (P&L, cohort, calibration, excursion, journal, universe, CF coverage, bot health) with `_safe_section()` failure-tolerance — runs in <1s; Session 28 + Session 28-followup investigated the 100% `partial_snapshots` rate and pushed `_SNAPSHOT_DEADLINE_SEC` from 90→300 (Outcome D — at Kalshi API ceiling; Session 28-2 architecture rewrite filed on watch list); Session 29 + Session 29-followup fixed a JSONDecodeError-forever silent corruption in `_journal_append` that had stopped journal writes for 27.5 hours, then verified all 4 event types resumed; Session 30 shipped the `live_momentum` research dataset + bucket analysis surfacing actionable findings (leader_price 60-70 wins at 100% +CLV at n=12 — supporting Session 19c's LM=0.65 change; challenger circuits over-perform their disabled-status). Three scheduled routines queued: **Apr 29 cursor-stability check** (today), **May 12 weekly-digest spot-check**, **May 18 LM re-validation**. See [Recent Improvements](#recent-improvements-apr-2025) below.
...
│   ├── tracker.py           # P&L tracking, market resolution, CLV settlement (idempotent), per-position MFE/MAE ratchet (Apr 24 Session 9). called_from kwarg for cadence telemetry (Apr 26 Session 17)
...
│   │
│   ├── live_watcher.py      # Per-game 10s-tick watcher — live_momentum + live arb; wp_edge proxy + dampened decision logging (Apr 24 Session 7); peak-tracking bug fix at line 2225 (Apr 26 Session 19a-peakfix — TRAILING_STOP can now physically fire)
...
│   ├── _report_helpers.py   # Shared utilities + 10 parameterized "shared section" renderers used by daily_report.py and weekly_report.py. _safe_section() failure-tolerance, JSONL streaming with malformed-line tolerance, traceback dedupe, gzip-uncompressed-size for archive comparisons (Apr 29 Session 35)
│   ├── daily_report.py      # Comprehensive daily markdown report — 10 sections, 24h ET window. Health pulse first, then scanner activity, decision audit (cohort), trade activity, CF coverage, live momentum events, DQS+regime distribution, cadence health, errors, state file growth. First I/O writes header so partial reports survive crashes. Output: bot/state/reports/daily/daily_report_YYYY-MM-DD.md (Apr 29 Session 35)
│   ├── weekly_report.py     # 16-section weekly synthesis — the 10 daily-shape sections at 7d + 6 weekly-only (week-over-week deltas, buckets, dataset rebuild summary, excursion + exit-replay, calibration findings, retuning candidates). Replaces weekly_digest.py from Session 24. Output: bot/state/reports/weekly/weekly_report_YYYY-WNN.md (Apr 29 Session 35)
│   ├── live_momentum_dataset.py # Per-tick decision dataset joining live_ticks + live_journal + clv into one tabular surface. One row per candidate decision (accept AND tunable-reject from Session 21 + 23). Forward returns at 30/60/120s + MFE/MAE in fixed window. Leakage property test enforced. Output: bot/state/research/live_momentum_dataset.csv (Apr 28 Session 30)
│   └── live_momentum_buckets.py # Markdown bucket reports across 7 single dimensions (sport, leader_price, dip, wp_edge, dqs, game_phase, spread) + 4 interaction tables. Marks n<5 as low-confidence. Authored Findings section (Apr 28 Session 30)
│
├── agent/
...
# 1,031 tests collected across 40 test files (Apr 27-28: +66 across Sessions 21, 23, 24, 28, 29, 30)
...
**Apr 27–29 — Pre-checkpoint coverage arc (Sessions 21, 23, 24, 28, 29, 30).** Continuation of strategy iteration explicitly framed around making the May 2 retuning checkpoint analytically rich. Driving principle (per user): "in paper mode, failure is the price of the data — collect everything we can, find an edge faster, make money sooner." Highlights: Session 21 closed Session 18 Finding #3's watch-but-no-enter black hole with 11-gate `skip_reason` instrumentation; Session 23 added live_momentum counterfactuals (mirrors Session 8's vig_stack pattern, ~10×/day rate, per-(sport,skip_reason)-per-day cap honored); Session 24 shipped `weekly_digest.py` so May 2's analysis is a 15-min review instead of 90-min synthesis; Session 28 + Session 28-followup hit the Kalshi API ceiling on universe enumeration (Outcome D — pushed deadline 90→300 with clear evidence of diminishing returns; Session 28-2 architecture rewrite filed on watch list, NOT shipped reactively); Session 29 + Session 29-followup fixed and verified a JSONDecodeError-forever silent-corruption bug in `_journal_append` that had stopped journal writes for 27.5 hours; Session 30 shipped the `live_momentum` research dataset + bucket reports surfacing leader_price 60-70 winning at 100% +CLV at n=12 (supporting Session 19c's LM=0.65 change from a totally different angle) AND challenger circuits over-performing despite their disabled status (worth follow-up investigation). Three scheduled routines queued: **Apr 29 cursor-stability check**, **May 12 weekly-digest spot-check**, **May 18 LM re-validation**.
