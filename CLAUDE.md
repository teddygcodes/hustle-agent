...
|---|---|---|---|
| `vig_stack_series` | `kalshi_series.py` | Mutually-exclusive ladders (weather, S&P ranges) where YES prices sum > 100¢. Buy the cheap NOs. Structural arb, no prediction. **Currently net loser** due to volatile-family ladders (hot-weather cities + fast-moving indices). Filter F stable families `KXHIGHMIA / KXHIGHAUS / KXINX` enter freely; volatile families require NO ≥ 0.93 (Apr 20, raised from 0.90 after bucket analysis showed only [92-96¢) is breakeven). **S144 (2026-05-16)** split the forecast gate by rung_kind: B keeps ±2°F inclusive (`FORECAST_NEAR_BUCKET_MARGIN`), T widens to 4°F strict less-than (`FORECAST_NEAR_THRESHOLD_MARGIN`) after 3 KXHIGHMIA T-side full-cap losses (~$600) at NWS delta=3°F. New decision reason `forecast_near_threshold` distinguishes T-blocks from B-blocks in `decisions.jsonl`. | 54 settled, **−$110.62**, 29W/25L (54%) |
...
**Session 144 (2026-05-16) addendum.** The "NO at 90-95¢ risk/reward" framing applies most acutely to **T-rung weather contracts**: open-ended ≥-threshold semantics give the market a long right tail — a forecast 3°F below the threshold still loses 100% of NO premium if the actual high reaches the threshold (Miami in May does this regularly). The 3 verified S144 losses (KXHIGHMIA T93→90°F, T92→89°F, T87 inferred) all sat in this geometry: NWS 3°F below T, NO entry 0.75-0.95, full-cap loss. Pre-S144 the forecast gate at [bot/strategies/vig_stack_series.py:563](bot/strategies/vig_stack_series.py:563) was asymmetric on T-rungs — it blocked above-threshold cases (forecast ≥ T − 2) but silently admitted below-threshold cases like the loss geometry (forecast = T − 3). The fix branches the gate on rung_kind: B keeps ±2°F inclusive (byte-identical), T uses `(T - forecast) < FORECAST_NEAR_THRESHOLD_MARGIN = 4` (strict less-than) which preserves above-threshold blocking AND adds below-threshold protection within margin. **Do NOT collapse the two margins back onto a single constant** — B-rungs are bounded above and below (1°F-wide); T-rungs are not. The gate sensitivity is structurally asymmetric. Battle Scar #9 (TP/SL exemption) untouched — fix is entry-gate-only.
...
**Session 143 (2026-05-16) addendum — third unwrapped call site fixed at [bot/live_watcher.py:3521](hustle-agent/bot/live_watcher.py:3521).** The Session 39 + Session 98 wraps addressed `snapshot_universe` inside `_main_loop`, but `scan_live_matches` (called every `LIVE_SCAN_INTERVAL=60s` from `_live_scan_loop`) was calling `agent.kalshi_client.get_markets(series_ticker=..., status="open", limit=200)` inside its `for sport, series in all_series.items():` loop without an executor wrap. Forensic evidence (2026-05-16 morning, `bot/state/forensics/2026-05-16-scan-wedge/`): 83 CLOSE_WAIT sockets to CloudFront atl58/atl59 edges in a 10h window; DNS confirmed `api.elections.kalshi.com → 3.163.101.5` (atl58 — matches captured forensic IP) — ruled out NWS (Akamai), ESPN (direct AWS), and TheRundown (Cloudflare) by DNS attribution. Per-iteration cost: 60s × ~5-7 active series × ~600 ticks/10h ≈ 3000-4000 calls; ~2-3% leak rate matches the 83-socket count. Fix wraps the call in `await loop.run_in_executor(None, lambda: get_markets(...))` mirroring the Session 39 pattern. No outer `asyncio.wait_for` because the per-call workload is bounded (single-page fetch, not a multi-page cursor walk). Regression test at [tests/test_live_watcher.py::test_scan_live_matches_runs_get_markets_via_executor](hustle-agent/tests/test_live_watcher.py).

**Session 148 (2026-05-17) addendum — fleet-wide audit closes the remaining 4 unwrapped sync entry points in `async _main_loop`.** S39 / S98 / S143 each closed ONE wrap site. Today's forensics (`bot/state/forensics/2026-05-17-secondary-leak/`) documented 4-5h wedges originating from 5 more files (scanner_weather, kalshi_series, scanner_sports_arb, position_monitor, clv) — all making sync Kalshi HTTP calls reachable from async `_main_loop` without an executor wrap. **Phase 0 surfaced a structural mismatch in the spec's premise:** the S143 leaf-wrap template assumed each leaf call sat inside `async def`, but Phase 0 verified all 5 spec-named files have **sync** leaf containers (`def scan_weather_markets()`, `def recheck_open_edges()`, `def scan_related_markets()`, `def check_clv_settlements()`, etc.). `await loop.run_in_executor(...)` is a syntax error inside `def`; the only minimal-change fix point was the 4 sync entry points in `_main_loop`. Wraps shipped at [bot/main.py:1323](hustle-agent/bot/main.py:1323) (`scan_cycle` — covers scanner_weather + kalshi_series + scanner_sports_arb), [:1373](hustle-agent/bot/main.py:1373) (`check_clv_settlements` — covers clv.py), [:1409](hustle-agent/bot/main.py:1409) (`recheck_open_edges` — covers position_monitor.py:41,158,233), and [:1451](hustle-agent/bot/main.py:1451) (`scan_related_markets` — covers position_monitor.py:415). The `scan_cycle` wrap explicitly overrode the prior "defer indefinitely" decision (Open Loops S143 Site 3) — Plan agent reviewed scan_cycle for thread-affinity hazards and found none (no `threading.local`, no `signal.*`, no `main_thread()` checks; state writes already lock-serialized via `bot/state_io.py`). **vig_stack hot path preserved:** vig_stack inside scan_cycle reads `buffered_universe` (snapshotted at [main.py:1320](hustle-agent/bot/main.py:1320) BEFORE the wrap) and makes no new HTTP, so the wrap moves only weather/series/sports-arb HTTP off the event loop without perturbing the +$1,064 vig_stack workhorse (Session 133). 8 regression tests added (`test_main_loop_runs_<entry_point>_via_executor` × 4 primary + `test_main_loop_continues_when_<entry_point>_raises_in_executor` × 4 companion); pytest 1831 → 1839. **Lineage:** S39 → S98 → S143 → S146 → S148 = the four-day infrastructure arc that should close the CLOSE_WAIT leak class permanently. If a same-shape wedge recurs post-S148, the diagnosis shifts to structural causes (executor exhaustion under burst, response.close() not called even in executor) rather than another missed wrap.

**Rule reinforcement:** any sync HTTP call inside an async-reachable code path — including sync helper functions called from `async def` coroutines or scheduled tasks — must be wrapped in `loop.run_in_executor`. CLOSE_WAIT socket leaks are the secondary symptom; the event-loop wedge is the primary symptom of Battle Scar #13. When adding a new scanner or watcher loop, grep for `requests.` / `import requests` in the helpers it calls; if any sync HTTP exists in the call graph, wrap at the bot's async boundary. **S148 closes the audit on existing entry points.** Any NEW async coroutine that calls into a sync helper that hits Kalshi must continue applying the wrap at the async boundary; the existing 6 wraps in `bot/main.py` (5 in `_main_loop`, 1 in `_position_check_loop`) plus 1 in `bot/live_watcher.py` are the canonical examples to mirror.

**Session 150 (2026-05-17) addendum — outer `asyncio.wait_for` at all 4 S148 boundaries.** S148's executor wrap moved sync I/O off the event loop but left each `await loop.run_in_executor(...)` unbounded. 2026-05-17 incident: post-S148 restart at 13:45 ET, first SCAN CYCLE completed 15:49 ET and entered the WEATHER branch — the executor thread then hung silently for 5h+ (15:49 → 21:07 ET still wedged). The wrap fixed the event-loop-blocking symptom but the wedge moved with the work: every async sibling task kept running (`_live_scan_loop`, `_heartbeat_loop`, etc.) while `_main_loop`'s scan section blocked on a wedged executor thread. S150 adds `asyncio.wait_for(..., timeout=_EXECUTOR_OUTER_TIMEOUT_SEC = 900)` at all 4 S148 sites at [bot/main.py](hustle-agent/bot/main.py): `scan_cycle`, `check_clv_settlements`, `recheck_open_edges`, `scan_related_markets`. 900s = one IDLE scan cycle (post-S109 default), tight enough to abort within one cycle of the next expected scan, loose enough to clear high-Kalshi-load scans without false-firing. Mirrors S98's `snapshot_universe` pattern at the next architectural boundary; closes BS#13 correctly as **wrap (S148) + bound (S150)**. On `asyncio.TimeoutError`: per-function `*_outer_timeout_count_24h` counter in `bot_state.json`, log error with attribution, fall through to existing recovery path (scan_cycle: `scan_failed=True` → 60s sleep; the other 3: default result to `[]` and continue). **Executor threads leak on timeout** (Python `concurrent.futures` can't cancel running threads); acceptable for transient wedges, watch-list auto-fires if a single counter hits ≥ 3/day sustained (open S151 to investigate the wedge root cause). 4 regression tests in `tests/test_main.py::test_main_loop_aborts_wedged_*_after_outer_timeout` mirror S98's `test_main_loop_aborts_long_snapshot_after_outer_timeout` at each new boundary. pytest 1839 → 1843. **WEATHER branch root cause inside scan_cycle (NWS fetch timeout path)** is deferred — see Open Loops; S150 is the band-aid, not the fix.
...
**Session 154 (2026-05-20) — root-cause resolution: the SECOND BS#17 hang (executor-thread join at process exit).** S58 (above) fixed watcher-cleanup *ordering*. A distinct hang remained: on a plain SIGTERM, `stop()` completes and logs "Bot stopped", but the process then hangs **after** `stop()` returns and needs `kill -9`. **Mechanism (Python 3.14, source-confirmed + clean-room + live repro):** the S39→S150 `run_in_executor(None, ...)` wraps (Battle Scar #13) moved sync Kalshi/NWS I/O onto the **default** ThreadPoolExecutor, whose workers are **non-daemon and cannot be cancelled** once running. At interpreter exit `asyncio.run()`→`Runner.close()` calls `loop.shutdown_default_executor(THREAD_JOIN_TIMEOUT=300)` — a **bounded 300s** wait — then the `concurrent.futures` `_python_exit` atexit handler `t.join()`s every worker with **NO timeout**. If any worker is mid-call at SIGTERM (the 397-position startup reconcile, a live-watcher tick, a scan, or a rate-limited Kalshi page — i.e. almost always on a running bot), the join blocks forever. S150's `wait_for(900s)` bounds the awaiting *coroutine*, not the *thread*, so it does nothing for shutdown. This is the FIRST link in the 5/14→5/20 cascade (wedge → orphan survives a missed kill-9 → duplicate runtime [BS#3] → concurrent sqlite writes → outcomes.db corruption → outage); S153 made the LAST link survivable, S154 severs the first.

**Fix (S154):** a bounded-shutdown watchdog. `_arm_shutdown_watchdog()` ([bot/main.py](hustle-agent/bot/main.py), near `_release_lock`) starts a **daemon** timer in the SIGTERM/SIGINT handler (`_on_shutdown_signal`, before scheduling `stop()`); if the process hasn't exited within `_SHUTDOWN_DEADLINE_SEC = 10.0`, `_force_exit()` calls `os._exit(0)` — which bypasses BOTH the 300s wait and the unbounded atexit join. The S58 graceful sequence is **preserved**; S154 only BOUNDS it. `_release_lock()` runs early in `stop()` (before the unbounded `notifier.stop()`), so the lockfile is clean before force-exit → the next child starts clean. On a clean shutdown the process exits before the deadline and the daemon thread dies with it. `_force_exit` is indirected so tests can patch it; arming is idempotent (SIGINT+SIGTERM both call it). Regression tests: `tests/test_main.py::test_shutdown_watchdog_force_exits_when_deadline_exceeded` / `test_shutdown_watchdog_stands_down_when_cancelled` / `test_arm_shutdown_watchdog_is_idempotent`.

**Verification (live, 2026-05-20):** clean-room repro (faulthandler showed the main thread parked in `_python_exit`→`t.join` with a wedged worker); the running bot wedged on a plain SIGTERM (survived >12s, "Bot stopped" logged then hung — exactly the kill-9 scenario); post-fix, **3 consecutive fully-started restarts** each self-terminated in ~10-11s with the `S154: graceful shutdown exceeded 10s` ERROR line and **zero kill -9**. **The watchdog fires on essentially every restart** — EXPECTED and correct, not "cleanup too slow": a running bot almost always has an uncancellable executor call in-flight at SIGTERM, so the join genuinely can't complete; a bounded 10s beats an infinite hang. Resolves the S149 SIGTERM-wedge HOLD. Deploy/restart discipline: signal the **python child** directly (`kill -TERM $(pgrep -P $(pgrep -f ".../run_bot.sh"))`), never the wrapper — per the S153 operational note, `kickstart -k` orphans the child. Cross-refs: BS#13/S150 (the wraps that created this), S153 (cascade survivability), BS#3/#14 (the duplicate-runtime cascade).
...
```python
{
  ...
  "current_date": str,                # YYYY-MM-DD; daily reset key for scans_today + telegram_throttled_count_24h + snapshot_outer_timeout_count_24h + 4 S150 *_outer_timeout_count_24h counters
  ...
  "scan_cycle_outer_timeout_count_24h": int,                  # reset with scans_today; forward-only since Session 150
...
- **S142 — Pattern 1 demotion threshold raised to N=4 in 14d (2026-05-15).** `tools/discovery_agent/heuristics/settlement_vs_rationale.py` `PATTERN1_TAIL_DEMOTE_THRESHOLD_14D = 4` widened the demote criterion from pre-S142 "`family_pnl_sum > 0 AND n_tail == 1`" to "`n_tail < 4`". Cluster strength, not family P&L direction, is what justifies HIGH severity; tail losses in net-positive families are structural cost per Battle Scar #10. **Phase 0 verdict:** the prompting `KXHIGHMIA-26MAY14-T93` (-$199.95) is the only post-S100 high-cap tail loss (cohort N=1 with `entry_price >= 0.85` AND `timestamp >= 2026-05-11`); empirical counterfactual sweep unreachable. Post-S100 cohort lose rate at `entry_price >= 0.85` is 6.2% (1/16 resolved) — below the 7-15% expected at 85-93¢ NO entry, so the gate is not letting bad trades through. ±2°F `FORECAST_NEAR_BUCKET_MARGIN` at [bot/strategies/vig_stack_series.py:563](bot/strategies/vig_stack_series.py:563) was NOT touched (Outcome C: HOLD). Post-S142 emission verified — `KXHIGHMIA-26MAY14-T93` now NOTABLE (was HIGH); `KXHIGHAUS-26MAY13-B90.5` stays HIGH at n_tail=4 (legitimate cluster). **Auto-fire trigger:** 2026-05-29 (14d) — re-measure HIGH-severity Pattern 1 emissions on currently-enabled stable families via `python3 tools/discovery_agent/main.py`. CONFIRM if HIGH emissions drop to ≤1; REGRESSED if still ≥3. Auto-fire if a 5th tail loss on a currently-enabled stable family fires before 2026-05-29 (demotion threshold wasn't tight enough). Auto-fire if a single stable family flips net-negative on a 30d rolling window (structural-cost framing is wrong; re-investigate `vig_stack_series` ENTRY gate). **Closed by S144 ENTRY-side fix on 2026-05-16** — the entry-gate widening (T-side 2°F → 4°F strict) is the upstream prevention; the S142 discovery-side demotion is the downstream noise reduction. Both ship together; auto-fire trigger still relevant for residual cases.

- **S144 — per-rung-kind forecast margin (2026-05-16).** `FORECAST_NEAR_THRESHOLD_MARGIN = 4` at `bot/config.py` (alongside `FORECAST_NEAR_BUCKET_MARGIN = 2`) widens the T-rung weather-forecast skip from ±2°F → 4°F strict less-than on the delta, after 3 KXHIGHMIA T-threshold full-cap losses in 11 days (~$600 total) where NWS was 3°F below the threshold. **Premise correction surfaced during Phase 0:** the pre-S144 condition `lo - 2 <= forecast <= hi + 2` was asymmetric on T-rungs because `_parse_weather_bucket` returns `(T, 999.0)` — it blocked above-threshold cases (forecast ≥ T − 2) but silently admitted below-threshold cases like the loss geometry. The new T-side condition `(T - forecast) < margin` preserves the above-threshold blocking AND adds below-threshold protection within margin. Branched gate at [bot/strategies/vig_stack_series.py:557-585](bot/strategies/vig_stack_series.py:557) on rung_kind from `bot/vig_stack_ladder_context._parse_strike`; new decision reason `forecast_near_threshold` + telemetry counter + `_VIG_STACK_GATES` entry keep B-side and T-side blocks distinguishable in `decisions.jsonl` / cohort_report. **Cross-ref S142 entry above:** S142 raised Pattern 1 demotion threshold to N=4 in 14d on the same loss cohort. S142 demotes the heuristic noise; S144 reduces the underlying loss frequency. Complementary. **Auto-fire trigger:** 2026-05-30 (14d) — re-measure `paper_trades.json` for (a) T-side full-cap losses on currently-enabled stable weather families (KXHIGHMIA, KXHIGHAUS) with entry ≥ 0.75 and `settled_at ≥ 2026-05-16`; (b) T-side accepts blocked by the new gate via `decisions.jsonl` (`reason=='forecast_near_threshold'`). CONFIRM if (a)=0 AND (b)≥2. REGRESSED if (a)≥1 with delta < 4 (margin insufficient — escalate to per-city/per-family map). Auto-fire if a 4th T-side full-cap loss on a currently-enabled stable family settles before 2026-05-30 regardless of timer. **Outstanding:** the 4 KXHIGHAUS B-bucket tail losses from S142 are a SEPARATE problem — existing ±2°F B-bucket margin should have blocked them but somehow didn't. Different gap, not addressed by S144. **S145 investigated; HOLD** — see next entry for Phase 0 findings.

- **S145 — KXHIGHAUS B-bucket forecast margin HOLD (2026-05-16, Outcome C).** Investigated widening `FORECAST_NEAR_BUCKET_MARGIN` from 2°F to address the 4 KXHIGHAUS B-bucket tail losses called out in S142/S144 (~$799 / 14d: 5/4 B82.5, 5/8 B80.5, 5/9 B89.5, 5/13 B90.5). Sister axis to S144's T-rung 4°F widening. **Phase 0 result: no MARGIN value clears the bar criteria** (block ≥3 of 4 losses, ≤30% wins blocked, net ≥ +$100/cohort). **Measured cohort (post-S100 KXHIGHAUS-B, N=9 settled, ladder_context populated):** 1 loss at `forecast_bucket_distance=5.0°F` (5/13 B90.5, fcst 85 vs bucket [90,91]); 8 wins at distances {3×5, 5, 7, 8}. Sweep table: MARGIN=2 blocks 0 / net $0; MARGIN=3-4 blocks 5 wins at dist=3 / net -$139.14 (no loss caught); MARGIN=5 blocks 1 loss + 6 wins / net +$49.86 (75% wins_blocked, fails ≤30% bar); MARGIN=7 blocks 1 loss + 7 wins / net -$3.27. **Cross-family at MARGIN=5° (Outcome A check):** KXHIGHMIA-B 100% wins blocked (1/1), KXHIGHNY-B 85.7% (6/7), KXHIGHDEN-B 75% (3/4) — global widening would devastate the workhorse +EV signals. **Outcome B (per-family widening) also fails** the KXHIGHAUS-internal ≤30% wins_blocked bar at MARGIN=5°F. **Architectural finding:** post-S100 KXHIGHAUS-B wins cluster precisely at `dist=3` (5/8 wins, all integer °F per NWS resolution), so any MARGIN ≥ 3°F that blocks the dist=5 loss necessarily blocks the dist=3 wins. The gate geometry forbids a clean margin-based fix at this data shape. **Pre-S100 data gap:** 3 of 4 losses (5/4, 5/8, 5/9) lack `forecast_bucket_distance` / `source_forecast_temp` (S100 deployed 2026-05-11; earlier trades have nulls). NWS archive lookup infeasible because trade dates (2026-05-03 → 2026-05-08) are forward-dated vs real wall-clock — external archives don't carry those dates. **Per spec's pre-committed kill rules (Strategy Termination Rules):** Outcome C HOLD documents the null, doesn't ship code, and sets a re-evaluation trigger. **Code unchanged.** `FORECAST_NEAR_BUCKET_MARGIN = 2` at [bot/config.py:486](bot/config.py:486) preserved. Inclusive `<=` boundary at [bot/strategies/vig_stack_series.py:588](bot/strategies/vig_stack_series.py:588) preserved (per user decision: B-side stays inclusive; S144's strict-`<` was T-side specific to its delta=4 boundary win evidence). **No restart, no test changes, no behavior change.** Closes the "Queued for future session" loop at the end of the S144 entry above on a HOLD decision rather than a code ship. **Auto-fire re-evaluation triggers:** (a) **2026-06-15** (30d) — re-measure with the post-S100 cohort extended; require N≥8 KXHIGHAUS B-losses with `forecast_bucket_distance` populated to re-attempt sweep; (b) auto-fire earlier if a 5th KXHIGHAUS B-loss with `forecast_bucket_distance ≥ 5` fires before 2026-06-15 (forecast error larger than observed); (c) auto-fire if KXHIGHMIA-B or KXHIGHNY-B settled count drops materially over 14d (other gate over-blocking — unrelated to S145 but adjacent watch); (d) auto-fire if `paper_trades.json` shows a B-rung loss at `forecast_bucket_distance < 3` on any currently-enabled stable family (existing ±2°F gate failure, would mean current MARGIN itself is too tight). **Alternate axes named for future investigation (NOT this session):** regime detection (Austin heat-dome / seasonal warming pattern — the 5/13 forecast 85→actual 90-91°F is consistent with NWS systematically underestimating peak temps on hot days); per-rung NO-entry price cap on KXHIGHAUS-B at high buckets (the 5/13 loss entered at $0.70 — a price ceiling at $0.75 would have admitted but reduced cap risk); ladder-rank-aware sizing (5/13 was selected_rung_rank_desc=2, just below the highest rung). None of these were measured in S145; documented to seed a future session if data accumulates.
...
- **S138 — universe partial-rate as operating regime (2026-05-13).** Phase 0.2 measured every day in the 17-day archive (2026-04-26 → 2026-05-12) plus today at **100% per-scan partial rate**, matching the Apr 30 baseline ([bot/state/reports/cursor_stability_2026-04-30.md](bot/state/reports/cursor_stability_2026-04-30.md): n=32, partial 100%). This is the S28-2-deferred steady-state, not a regression. The brief premise ("44% today" from `bot_state.partial_snapshots_today / total_snapshots_today` = 170/387) is a counter accounting artifact — the true per-scan rate from `bot/state/universe.jsonl` is 100% (46/46). Per-snapshot pages today: ~700-733, matching S28-followup's expectation at the 300s inner deadline ([bot/universe.py:93](bot/universe.py:93)). `snapshot_outer_timeout_count_24h` = 0; the 600s outer guard ([bot/main.py:1290](bot/main.py:1290), Battle Scar #13 addendum) is not firing. Active trading paths (`vig_stack_series`, `live_momentum`) do not read `universe.jsonl` — confirmed. **Why:** the S28-2 per-series-paginated rewrite remains the architectural unblock; the deadline-bump path is at its ceiling (90 → 180 → 300; further bumps diminishing per S28-followup analysis). **How to apply:** when [CLAUDE.md §12 WARN](#12-botstatebot_statejson--universe-partial-rate-ratio-session-155-reframed-session-138-2026-05-13) fires, do NOT investigate on the WARN alone. Investigate only when (a) `snapshot_outer_timeout_count_24h ≥ 3` in any 24h window, or (b) per-scan partial rate exceeds 70% AND on rising trend across ≥3 consecutive days (which would itself signal degradation beyond today's 100% steady-state, mathematically requires cursor-rows-per-scan dropping materially). **Auto-fire trigger:** 2026-05-27 — re-measure trailing 7-day per-scan partial rate via the S138 Phase 0.2 script (in `~/.claude/plans/session-138-investigate-shiny-hanrahan.md`); if any day exceeds 70% partial OR if `snapshot_outer_timeout_count_24h ≥ 3` fires in any 24h window between now and then, escalate.

- **S138 follow-up: silent downstream consumer partial-flag handling.** Four tools read `bot/state/universe.jsonl` without filtering or caveating `partial=true` rows: `tools/discovery_agent/heuristics/universe_gap.py` (line 67-89), `tools/discovery_agent/heuristics/concurrent_attack_angles.py` (line 237-254), `tools/strategy_lab/data_loader.py` (line 64-76 `iter_universe`), `tools/backtest.py` (line 84-114 `load_universe_snapshots`). At the documented 100%-partial operating regime, all four propagate partial-snapshot bias into discovery and back-test outputs without operator visibility. The correct pattern already exists in `tools/universe_report.py` (line 19-21 docstring + line 126-127, 154-156 implementation) and `tools/_report_helpers.py` (line 506-514, 1042-1070): filter partial rows from absolute counts, surface `partial_pct` in output, retain partial rows only where they're meaningful (e.g. captured slice for partial scans). **Why:** consumers silently bias their outputs; the bias is not catastrophic at today's regime (active trading is unaffected, daily/weekly reports caveat correctly), but the four silent consumers create the failure mode where a future operator sees a "discovery gap" report and acts on a partial-scan artifact. **How to apply:** ship as **four separate one-tool-per-commit follow-ups** per the 1-value-per-commit discipline. Per-tool acceptance criteria: (1) `partial=true` rows excluded from absolute counts in tool output, (2) percentage of partial rows in the windowed corpus reported in the tool's output preamble, (3) test fixture extended to exercise both partial-rich and partial-free inputs (current test fixtures generate synthetic universe.jsonl with NO `partial=true` rows). No bot restart required; doc-tool changes only.

- **S138 follow-up: universe-metering counter semantics fix.** `bot/universe.py:_record_partial_meter` ([line 389-417](bot/universe.py:389)) is called on every `snapshot_universe` invocation, including the early-exit / zero-row paths where `partial=False` stays default — inflating `total_snapshots_today` relative to the count of scan_ids actually written to `universe.jsonl`. As of 2026-05-13, divergence is ~8.4× (387 counter denominator vs 46 scan_ids written). Per-scan partial rate from `universe.jsonl` is 100%, but the counter shows 43.9%. **Why:** the counter is what operators see first (CLAUDE.md §12 "Inspect" command); S138's own brief was misled by the 43.9% number, which is why the §12 reframe above adds the universe.jsonl-based inspect command as authoritative. **How to apply:** scope to a separate session because (a) fixing the counter changes the schema interpretation of two `bot_state.json` keys (forward-only field rules in CLAUDE.md "Canonical Data Schema Reference" apply), and (b) S138 explicitly committed "no schema change" in its plan scope. Defer until either (1) a daily-report consumer surfaces the counter as user-visible in a way that forces the fix, or (2) the S138 auto-fire trigger fires on 2026-05-27 and re-investigation needs the corrected counter. Candidate fix: only call `_record_partial_meter` from the row-emitting success path (after `_BUFFER[scan_id] = rows` at [line 372](bot/universe.py:372)); leave early-exit zero-row paths uncounted; backfill semantics in CLAUDE.md schema reference with a forward-only "rename event" note.

- **(Resolved by S148, 2026-05-17)** — S143 deferred Site 2 (`bot/scanner_weather.py:116`) and Site 3 (`scan_cycle()` architectural wrap) both closed by S148. Phase 0 verified that all 5 spec-named files (scanner_weather, kalshi_series, scanner_sports_arb, position_monitor, clv) have **sync** leaf containers — the S143 leaf-wrap pattern was structurally inapplicable. Wrap moved to the 4 sync entry points in `async _main_loop` (Shape A): `scan_related_markets` (line 1451), `check_clv_settlements` (line 1373), `recheck_open_edges` (line 1409), and `scan_cycle` (line 1323). The scan_cycle wrap covers Site 2 (scanner_weather) + kalshi_series + scanner_sports_arb collectively. Plan agent reviewed scan_cycle for thread-affinity hazards — none found; vig_stack reads `buffered_universe` (no new HTTP) so the wrap doesn't perturb the +$1,064 workhorse. The deferral's unblock condition ("Site 2's targeted fix proves insufficient OR a third unwrapped scanner-side `get_markets` call is added") was satisfied by 2026-05-17 forensics showing 4-5h wedges originating in these unwrapped paths. Mirrors the S39 + S98 pattern. +8 regression tests in `tests/test_main.py` (4 primary asserting non-MainThread + 4 companion asserting executor-propagated exception handling). pytest baseline 1831 → 1839.

- **S150 — WEATHER branch root cause inside `scan_cycle` deferred (2026-05-17).** S150 shipped the outer `asyncio.wait_for` guard at all 4 S148 boundaries (constant `_EXECUTOR_OUTER_TIMEOUT_SEC = 900` at [bot/main.py](hustle-agent/bot/main.py); 4 new per-function counters in `bot_state.json`). **S150 is the bounded-wedge band-aid, not the root-cause fix.** The 2026-05-17 wedge originated in the WEATHER branch of `scan_cycle` (likely NWS fetch path in `bot/scanner_weather.py` — Akamai-fronted, separate from the Kalshi 429 storm class). Root cause investigation deferred because (a) the 4 S148 forensic wedges (`bot/state/forensics/2026-05-17-secondary-leak/`) point at the wrap boundary, not at a specific NWS call signature; (b) WEATHER branch refactor risks perturbing the `vig_stack_series` workhorse (+$1,064 / 169 trades / 82% WR post-S100) which depends on cached NWS forecasts for the forecast-near-bucket gate; (c) S150's per-function counters give forensic attribution that the prior single-counter pattern lacked. **Auto-fire trigger:** if `scan_cycle_outer_timeout_count_24h ≥ 3/day` for 2+ consecutive days, open S151 focused investigation (canonical targets: NWS fetch timeout, `kalshi_series` slow path, `scanner_sports_arb` slow path — `scan_cycle` is the umbrella). If any single `*_outer_timeout_count_24h ≥ 1/h` sustained, open S151 urgently — executor pool exhaustion risk as leaked threads accumulate (Python `concurrent.futures` can't cancel running threads); fix candidates: tighten timeout to 600s, OR add `executor.shutdown() + recreate` pattern, OR per-request HTTP timeouts at the NWS / Kalshi client level. If scan cadence drops below 4 cycles/h sustained after guard fires, the bounded wedge is still too much — tighten or investigate. **Watch-list re-evaluation:** 2026-05-18 ~21:30 ET (24h post-ship): all 4 `*_outer_timeout_count_24h ≤ 2/day` AND scan cadence at IDLE 15-min sustained = CONFIRM. **S151 update (2026-05-18):** S151 investigated the cadence-collapse trigger and ruled OUT the WEATHER branch as the cause — `scan_cycle_outer_timeout_count_24h = 0` across the 23h post-S150 window. The cadence collapse traced to `check_clv_settlements` (3,528-record sequential-iteration wedge under rate-limiter physics; see new S151 entry below) compounded by macOS laptop sleep. The S150 WEATHER deferral remains active and the auto-fire triggers above are unchanged — S151 is a different finding, not a closure of S150's WEATHER work.

- **S151 — 5 additional unwrapped sync I/O sites in `async _main_loop` (2026-05-18).** Phase 0.4 audit during S151 surfaced 5 sync calls in the same coroutine that the S148 audit missed — all BS#13 violations of "any sync HTTP call inside an async-reachable code path must be wrapped in `loop.run_in_executor`": `check_fills()` at [bot/main.py:1488](hustle-agent/bot/main.py:1488) → [bot/executor.py:1257](hustle-agent/bot/executor.py:1257); `update_positions(called_from="_main_loop")` at [bot/main.py:1498](hustle-agent/bot/main.py:1498) → [bot/tracker.py:85](hustle-agent/bot/tracker.py:85); `check_trailing_stops()` at [bot/main.py:1634](hustle-agent/bot/main.py:1634) → [bot/executor.py:1505](hustle-agent/bot/executor.py:1505); `check_mm_fills()` at [bot/main.py:1648](hustle-agent/bot/main.py:1648) → [bot/market_maker.py:314](hustle-agent/bot/market_maker.py:314); `scan_market_making_opportunities(...)` at [bot/main.py:1672](hustle-agent/bot/main.py:1672) → [bot/market_maker.py:66](hustle-agent/bot/market_maker.py:66). None were the DOMINANT cause of the post-S150 cadence collapse (`check_clv_settlements` at 176-min worst case dwarfed any of these), but they are real wraps that should ship to bring `_main_loop` from 4 wrapped + 5 unwrapped sites to 9 wrapped + 0 unwrapped, mirroring the S148 audit shape. **Why deferred:** S151 followed "1 logical fix per commit" discipline — shipping the check_clv batch cap was the smallest impactful change for today's cadence collapse. Wrapping these 5 sites is a separate logical change that needs its own Phase 0 verification (each call's Kalshi-touch surface, whether any have thread-affinity hazards like `update_positions` which mutates `positions.json`, regression test coverage). Per BS#13: "the existing 6 wraps in `bot/main.py` (5 in `_main_loop`, 1 in `_position_check_loop`) plus 1 in `bot/live_watcher.py` are the canonical examples to mirror" — applying that pattern adds 5 more wraps to `_main_loop`. **Auto-fire trigger:** open the next session to ship these 5 wraps IF (a) post-S151 CLOSE_WAIT count rises above 50 sustained for 24h (the leak class hasn't fully closed; one of these 5 sites is contributing), OR (b) `bot.lock` mtime ages stale by >120s during scan steps (event-loop blocking by one of these 5 sites), OR (c) by 2026-06-01 (14d post-S151) regardless of metrics, on the principle that BS#13 audit completeness is worth shipping while the context is fresh. **Each wrap is a 5-line change** (mirror the S148 pattern), so all 5 fit in a single session with ~10 regression tests (each site: non-MainThread + exception-propagation assertion).
...
- **S155 lead — CLV backlog drain (group-path terminal-marking) + malformed-ticker emitter (2026-05-21).** S155's Phase 0 found 1,097 dead CF records that 404 forever: `KXHIGHNY-26APR*` (n=1,025) + `KXTEST*` (n=72). S155 dead-marks the *shape-detectable* malformed records up-front (11 in the first live cycle), but the GROUP settlement path structurally CANNOT terminal-mark the ~1,026 *groupable* stale 404/`scalar` records (median age 9.5d) — only the per-record fallback 404-marks, and groupable records skip it — so they persist as open, clog the oldest-first drain head, and starve settleable groups past the 600s budget (this is why the S155 drain is correct but slow, ~2-3/cycle). **Next CLV session (actionable):** group-path terminal-marking for groupable 404s + `scalar`-result handling, draining OPEN toward its ~1,125 futures-floor. Those dead records are the *symptom*; the *emitter* is the root. The *emitter* is unidentified: why did CF emission produce 1,025 malformed `KXHIGHNY-26APR*` rows (a single dead event) and any `KXTEST*` debug residue in production `clv.json`? Leads: (a) does `bot/clv.py` CF emission validate ticker shape / event liveness before recording? (b) is there a scan path emitting CFs for already-closed or test events? (c) the "Check the Data §2 pollution check" already flags `entry_price_cents < 3` and `KXTEST` — extend it to flag malformed event tickers. **Why it matters:** 1,097 dead records ≈ 27% of the open-CF set; if the emitter keeps producing them, `settlement_failed` accumulates, and CF-data-quality degrades for any analysis reading `counterfactual_*` records (e.g. the S156 weather-floor cohort). Also: Kalshi returns `result="scalar"` for some markets which settlement skips (rare; benign; noted for completeness). **No auto-trigger;** investigate when an operator has bandwidth, or if `settlement_failed` count rises >200 in any 7-day window.

- **S156 — per-family vig_stack weather-floor loosen HOLD (2026-05-21, Outcome C).** Investigated loosening `VIG_STACK_WEATHER_MIN_PRICE` 0.93→~0.80 for KXHIGHNY + KXHIGHDEN on settled-CF evidence that the [80,93) NO band is +EV. **Phase 0:** both families pass every quality criterion (KXHIGHNY mean CLV +14.8c / WR 100%; KXHIGHDEN +12.1c / WR 95%; median≥mean both; real ≥0.93 paper net-positive +$174.42/29 and +$228.47/31) but FAIL the pre-committed **unique-day-N≥30** floor — band unique-day-N = **10 (KXHIGHNY) / 21 (KXHIGHDEN)** after S86 `(ticker,day)` dedup deflated raw 38/109 by ~4.7×. N-thin, not ruled-out (planner independently reproduced 10/21). Only real money in the band is negative (−$8.46/3, −$51.41/9) but pre-S36 auto-cut-loss era → confounded. KXHIGHCHI band correctly −EV (−8.3c), stays disabled. **Code unchanged:** `VIG_STACK_WEATHER_MIN_PRICE=0.93` ([bot/config.py:635](bot/config.py:635)), `VIG_STACK_FAMILY_FLOOR_OVERRIDES={}` ([bot/config.py:662](bot/config.py:662)), gate at [bot/strategies/vig_stack_series.py:661-678](bot/strategies/vig_stack_series.py:661) untouched. No restart, no test changes. **Auto-fire re-evaluation triggers:** (a) **2026-06-20** (30d) — re-run the S156 Phase 0 (3 read-only scripts in `~/.claude/plans/session-156-per-family-velvet-micali.md`) and re-apply the bar **unchanged**; (b) auto-fire earlier the moment unique-day-N≥30 accumulates for either family in [80,93) (gated on `check_clv` settlement throughput, slow post-S155 per the lead above); (c) **auto-KILL** the loosen idea entirely if any new floor-era ≥0.93 KXHIGHNY or KXHIGHDEN cohort flips net-negative on a 30d rolling window (family degrading → the CF +EV is moot). **Fix-if-clears (reuse, not new constant):** add `"KXHIGHNY":0.80` / `"KXHIGHDEN":0.80` to the existing `VIG_STACK_FAMILY_FLOOR_OVERRIDES` (already wired into the volatile gate via `get_vig_stack_floor_for_family`); zero gate-site change. **Adjacent observation (out of S156 scope):** this gate also floors sports futures (KXNBA/KXMLB/KXNHL — "not stable" → 0.93), a different question from weather-forecast variance; those CFs are unsettled (settle months out), so no signal today. Battle Scar #9/#10, S144 forecast gates, S53 family caps untouched.
...
6. **§7 Anomalies + Watch-List Status.** Anomalies first (CRITICAL/WARN/INFO entries inline with prefixes), then per-entry watch-list status (TRIGGERED / NOT_YET_TRIGGERED / MANUAL_CHECK_REQUIRED). Session 91 sub-feature 6 absorbed the old §9 auto-flags here (`daily_report_stale`, `watchlist_manual_checks` count, `watchlist_triggered` count). Session 91 sub-feature 1 retires obviously-stale entries (≥30d MANUAL_CHECK_REQUIRED with no recurrence) into `bot/state/watchlist_resolved.json`. **Session 147 (2026-05-17) added a 5th retirement criterion: axis explicitly ruled out by subsequent session.** If a planner observes a TRIGGERED (or MANUAL_CHECK_REQUIRED) watch-list entry where the underlying axis has been investigated and ruled out in a subsequent session (Pattern C in a sweep, parent sport disabled, substrate-gap finding, etc.), append an entry to `bot/state/watchlist_resolved.json` with `manual_axis_ruled_out: true` (required — bypasses S91's reversibility-first design at [tools/glint_status.py:801-832](tools/glint_status.py:801)), `reason: "axis_ruled_out"` (or `parent_axis_closed` / `firing_confirmation_closed` for the precise sub-case), `resolved_by: "S<N>"`, and a `rationale` citing the closing session(s) by ID + finding (e.g. "S41 Pattern C + S129 confirmation"). Mirrors the S91 30-day-stale criterion on semantic grounds instead of temporal. Auto-fire re-open conditions: if a disabled sport is re-enabled (revisit wta-CF retirements), if a substrate ships that changes the closed axis's question (revisit live_momentum retirements), or if §7 TRIGGERED count rises above 2 in any future dashboard read (re-run the S147 protocol).
...
### 12. `bot/state/bot_state.json` — universe partial-rate ratio (Session 15.5; reframed Session 138, 2026-05-13)
- **Inspect (bot_state counter):** `python3 -c "import json; s=json.load(open('bot/state/bot_state.json')); t=s.get('total_snapshots_today',0); p=s.get('partial_snapshots_today',0); print(f'snapshots today: {t}, partial: {p} ({100*p/t if t else 0:.1f}%)')"`
- **Inspect (true per-scan rate from universe.jsonl):**
  ```bash
  python3 -c "import json; from collections import defaultdict; s=defaultdict(lambda: False); [s.__setitem__(json.loads(l).get('scan_id',''), s[json.loads(l).get('scan_id','')] or bool(json.loads(l).get('partial'))) for l in open('bot/state/universe.jsonl')]; p=sum(1 for v in s.values() if v); print(f'scans today: {len(s)}, partial scans: {p} ({100*p/max(1,len(s)):.1f}%)')"
  ```
- **Operating regime (NOT a regression):** the per-scan partial rate has been **100%** every day since at least 2026-04-26 — verified Session 138 Phase 0.2 across the 17-day archive plus today. This is the S28-2-deferred steady-state, not an alert condition. The decision is canonical: *"accept that partial=True is the new normal under current Kalshi load"* (Session 28-2 in [CLAUDE-sessions.md:1297-1310](CLAUDE-sessions.md)). The inner cursor deadline at [bot/universe.py:93](bot/universe.py:93) is **300s** (bumped 90 → 180 → 300 across S15.5 / S28 / S28-followup); at 300s the cursor reaches ~720 pages/scan, which is short of the Kalshi `status=open` universe end. The Apr 30 baseline ([bot/state/reports/cursor_stability_2026-04-30.md](bot/state/reports/cursor_stability_2026-04-30.md): n=32, median 1949 cursor_rows, cadence 32s, partial 100%) matches today's regime exactly.
- **bot_state counter caveat (S138 finding):** `partial_snapshots_today / total_snapshots_today` does **not** equal the per-scan partial rate. As of 2026-05-13, the counter reads 170/387 ≈ 43.9% while every emitted scan in `universe.jsonl` is partial (46/46 = 100%). The divergence is structural: `_record_partial_meter` at [bot/universe.py:380](bot/universe.py:380) fires on every `snapshot_universe` invocation including the early-exit / zero-row paths where `partial=False` stays default — those calls inflate the denominator without ever being persisted to `universe.jsonl`. **The universe.jsonl count is authoritative**; the bot_state counter is preserved unchanged for forward-only schema compatibility (S138 made no schema change). See "Open Loops → Operational hygiene not yet shipped" for the counter-semantics-fix watch-list entry.
- **WARN signal:** `bot/universe.py:419-425` logs `universe partial rate elevated: N% over last 10 snapshots` when the trailing 10-snapshot window ≥ 10%. **The 10% threshold is aspirational, not the operating baseline.** Sustained WARN at 100% is the documented steady-state and is NOT alert-actionable on its own.
- **Escalation triggers (the actual "stop, investigate" signals):**
  - `bot_state.snapshot_outer_timeout_count_24h ≥ 3` in any 24h window — the 600s outer guard at [bot/main.py:1290](bot/main.py:1290) (Session 98, Battle Scar #13 addendum) is firing, which is a different and worse failure mode than the 300s inner deadline. Currently 0.
  - Any single day's per-scan partial rate exceeds 70% AND is on a rising trend across 3+ consecutive days, signalling degradation past today's 100%-steady regime would actually require new instrumentation (e.g., cursor-rows-per-scan dropping materially) — re-run S138 Phase 0 measurement set.
- **Downstream consumer awareness:** four tools read `universe.jsonl` without filtering `partial=true` rows (`tools/discovery_agent/heuristics/universe_gap.py`, `tools/discovery_agent/heuristics/concurrent_attack_angles.py`, `tools/strategy_lab/data_loader.py`, `tools/backtest.py`). Daily/weekly operator reports (`tools/_report_helpers.py` → `daily_report.py`/`weekly_report.py`) and `tools/universe_report.py` correctly filter+caveat. Active trading paths (`vig_stack_series` via `get_buffered_markets`, `live_momentum` via its own Kalshi+ESPN fetch) do **not** read `universe.jsonl` and are unaffected. See Open Loops for the silent-consumer follow-up.
...
settlement_vs_rationale:
  ...
  Pattern 1 (tail_loss_in_high_cap_family)     → only when family aggregate is net-negative AND n_tail >= 2 in 14d window (Session 67 refinement). Session 142 also tightened the agent-side severity ladder: findings with n_tail < 4 in 14d window are auto-demoted below HIGH, so any HIGH-severity Pattern 1 finding already meets a stronger cluster bar than the operator-level promotion rule requires.
