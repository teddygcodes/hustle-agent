...
- **S133 vig_stack_futures lean-in resolved by S134 (2026-05-13); per-game scope simulation CLOSED by S159 (2026-05-21, ruled out).** S134 investigated the S133 KXMLBGAME surface. Result: **do NOT add `vig_stack_futures` to `_VIG_STACK_OPP_TYPES` on current evidence.** The N=10 cohort remains +$264.65 / +$26.47 mean, but the signal is concentrated: median -$0.59 and total excluding the top winner -$32.27. The decisive finding is early-exit counterfactual: 6 EE trades actual +$461.45 vs held-to-settlement +$213.00, delta +$248.45, so auto-exit on futures is currently useful. Mechanism: nearest universe snapshots reconstruct real two-rung per-game YES-sum >100c ladders, but realized P&L is dominated by game outcome and auto-exit timing. **Open follow-up scope:** simulation-only NBA/NHL per-game vig_stack replay with explicit futures auto-exit policy preserved. Evidence: KXNBAGAME has 6,890 universe rows / 439 scan IDs / 0 vig_stack decisions and rough post-Apr-29 YES-sum>100 rate 97.1%; KXNHLGAME has 5,418 rows / 436 scan IDs / 0 vig_stack decisions and 95.3% YES-sum>100. Do not ship scanner expansion directly from rough counts; replay/simulate first. **[CLOSED by S159, 2026-05-21 — ruled out]:** the replay (`tools/sim_pergame_vig_stack.py`, 6,675 pairs) found the per-game vig is a SPREAD ARTIFACT — at mid the 2-outcome YES-sum is exactly 100¢ (no overround by construction; a 2-outcome market is A + not-A), the capturable `NO_ask_sum<100` gate fires on only 3.7% (NBA) / 2.0% (NHL) of pairs at a guaranteed −1¢, and after Kalshi fees nets negative (NBA Σ −59¢/24, NHL Σ −28¢/12; median + ex-top both <0). The Outcome-D TOTAL/SPREAD ladders aren't vig_stack shape either (nested `total≥N` thresholds / overlapping by-N rungs that price >1 winner). Per-game winners are NOT a vig target; no scanner expansion shipped; Battle Scar #9 futures-exemption decision moot. See the S159 ☑ block. **S160 (2026-05-21) closed the sibling commodity-ladder prospect identically** (KXWTI/KXSILVERD/KXAAAGASD/KXGOLDD are cumulative *above-$X* thresholds, NOT single-winner partitions: median 8-22 simultaneous-YES rungs vs KXINX's ~0; differenced synthetic partition Σmid=100¢ → spread artifact). **Strategic conclusion (S159+S160):** vig_stack's edge does NOT generalize to adjacent Kalshi families — it's structural to weather/index (real *mid*-overround on mutually-exclusive buckets). Stop prospecting vig_stack-shaped expansions. The remaining frontier is genuinely-new strategy CLASSES (`post_event_reversion`, auto-fires 2026-06-11), and offline realized-P&L sim is settlement-blocked (the offline-sim-settlement-gap memory) — so validating a NEW edge needs shadow-mode (Future Direction, 0% built) or live data, not offline replay.
