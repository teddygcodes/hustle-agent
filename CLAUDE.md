...
- **S133 — vig_stack_futures lean-in investigation (Operating Posture surface).** Session 133's vig_stack health re-check surfaced `vig_stack_futures` as the highest-per-trade vig_stack class: N=10 KXMLBGAME-prefixed trades, total +$264.65, mean +$26.47/trade, 50% WR (PnL>0). Bootstrap 95% CI [−$66.48, +$123.24] spans 0 — magnitude real, sign noisy on N=10. Individual trade swings of ±$200+ are typical (Apr-29 +$172.52, May-5 −$199.88, May-6 +$296.92, May-8 −$199.68, May-9 +$204.30). The Apr-30 +$172 KXMLBGAME finding documented in "Operating Posture" above is no longer a singleton — it's been joined by 2 larger winners and 2 large losers. Per CLAUDE.md Operating Posture, the right move is investigation, not lock-in or fix. **Candidate follow-up scope:** (1) is the +EV concentrated in 1-2 trades or genuinely distributed? Per-trade decomposition + per-game-state breakdown. (2) does the variance reflect MLB-game-winner mispricing structurally, or a sizing/exit-timing artifact? (3) **Per Operating Posture "where else could this work?": are NHL/NBA/NFL per-game winners similarly underscanned but profitable?** The bot's scanner currently treats per-game sports markets as `vig_stack_futures` only when the ladder math fires; if other per-game families would also trigger but aren't being scanned, that's the next surface. **Auto-trigger:** when `vig_stack_futures` cohort reaches N≥15 OR total accumulates beyond +$500 or below −$200, promote to a focused investigation session. Until then, this entry stays open. Note: `_VIG_STACK_OPP_TYPES = ("vig_stack_no", "vig_stack_series")` at [bot/main.py:73](bot/main.py:73) deliberately EXCLUDES `vig_stack_futures` per CLAUDE.md Operating Posture — auto-exit on futures is preserved-as-investigated, not preserved-as-bug; do not "fix" the exemption set without first investigating the mechanism.
