...
- **S143 deferred — Site 2: `bot/scanner_weather.py:116` unwrapped get_markets.** Same failure mode as the Session 143 Site 1 fix (sync `agent.kalshi_client.get_markets` inside an async-reachable call stack via sync `scan_weather_markets` → sync `scan_cycle` → async `_main_loop` at [bot/main.py:1323](bot/main.py:1323)), lower frequency (~30 calls/10h vs Site 1's 3000-4000). Wrap in `loop.run_in_executor` mirroring the Session 143 pattern. Scope: either convert `scan_weather_markets()` to `async def` and wrap there, or wrap each `get_markets` invocation at its call site. Sister-site audit: re-grep `bot/` for any other direct `get_markets(...)` calls reachable from async via sync helpers. **Auto-fire trigger:** if 24h post-S143 watch-list shows CLOSE_WAIT count >5 to CloudFront atl58/atl59 edges, escalate immediately — Site 2 alone could plausibly contribute that residual. **Defer trigger:** CLOSE_WAIT count drops to <5 at 24h → ship Site 2 in a separate session within 7 days regardless (correctness fix, not urgency fix).

- **S143 deferred — Site 3: `bot/scanner.py` scan_cycle() architectural wrap.** `scan_cycle()` is synchronous but called from async `_main_loop` at [bot/main.py:1323](bot/main.py:1323) without an executor wrap. scan_cycle internally invokes weather scanner, sports arb, and vig_stack (the latter uses buffered universe — no new HTTP). Wrapping scan_cycle entirely in `loop.run_in_executor` would fix Site 2 as a side effect plus any future sync-HTTP sites added inside scan_cycle. Risk: scan_cycle may have thread-affinity assumptions (state file writes via `state_io.py` are lock-protected so likely safe, but full audit needed). **Defer indefinitely:** revisit only if Site 2's targeted fix proves insufficient OR a third unwrapped scanner-side `get_markets` call is added. Higher blast radius than current evidence warrants — vig_stack is the +$1,064 post-Apr-20 workhorse (Session 133) and shouldn't be perturbed for a watcher-loop leak.
