...
- **S105 — Cross-platform settlement matcher.** Validation corpus doesn't exist publicly (only LLM-based products like Predexon, which warn about hallucinations). Source: [`docs/superpowers/specs/2026-05-11-cross-platform-matcher-design.md`](docs/superpowers/specs/2026-05-11-cross-platform-matcher-design.md). **S116 (2026-05-12) shipped the scraping + candidate-generation infrastructure** — `tools/build_cross_platform_corpus.py` produces a labeling queue at `bot/state/cross_platform_labeling_queue.jsonl` (5,000 top-Jaccard pairs from a 3-day scrape: 2,937 suggested-MATCH + 2,063 suggested-NEEDS_REVIEW; full 604,603-pair corpus cached at `bot/state/cache/cross_platform_candidate_pairs_full.jsonl`, gitignored). **S117 (2026-05-12) shipped matcher v1 + validation harness** — `bot/cross_platform_matcher.py` is pure deterministic code; `tools/validate_cross_platform_matcher.py` evaluates the queue and writes priority heuristic-disagreement pairs to `bot/state/matcher_heuristic_disagreement_pairs.jsonl`. **S118 (2026-05-12) validated the NO_MATCH direction against 40 Codex-labeled disagreement pairs** — all top-40 priority disagreements had resolved-outcome divergence and were labeled `NO_MATCH` with `labeler: "codex"`, per-pair reasoning, and outcome cross-check metadata; `python3 tools/validate_cross_platform_matcher.py --labeler codex` reports 0 false negatives on the S118 NO_MATCH-direction slice. **S119 (2026-05-12) validated the MATCH direction against 40 stratified MATCH_HIGH_CONFIDENCE samples (20 boundary + 20 interior) — found 24 false positives.** Outcome B: matcher v1 has structural failure modes the threshold alone cannot fix. (1) Same fixture, different bet type (match-winner vs O/U vs exact-score vs map-handicap vs set 1/set 2 winner; ~20 FPs). (2) Crypto hourly snapshot (Kalshi at hh:mm EDT) vs end-of-day window (Polymarket "on May N"; ~4 FPs). Hard rule (DIVERGED + MATCH → NO_MATCH) never fired, as predicted by the matcher's upstream `resolved_outcome_conflict` guard at [bot/cross_platform_matcher.py:207-214](bot/cross_platform_matcher.py:207). Per-stratum: boundary-20 = 85% FP rate at Jaccard=0.60 threshold floor; interior-20 = 35% FP rate. Combined S118+S119 (80 Codex labels): accuracy 67.5%, FP=24, FN=0. **Matcher v1 NOT shipped as live-arb-validated. S120 must address bet-type discrimination + settlement-window granularity in MATCH classification**, validated against the 80 combined Codex labels (S118+S119) as ground truth. Regression test [tests/test_validate_cross_platform_matcher.py::test_matcher_classifications_locked_against_codex_labels](tests/test_validate_cross_platform_matcher.py) locks the S119 baseline (FP=24, FN=0); S120 must update the assertion when matcher is tuned. Caveat: Codex labels still pending Claude/operator spot-check before being treated as fully trusted ground truth, but the failure pattern is structural and reproducible; do not need spot-check to queue S120 work. **Why it matters:** the matcher's positive classifications are not yet safe for live arb — running the bot against MATCH_HIGH_CONFIDENCE today would attempt arb on bet-type mismatches and produce ~$24 of expected loss per 40 attempted arbs (60% FP rate × $1 expected loss per FP).
