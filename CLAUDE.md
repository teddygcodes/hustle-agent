...
- **S157 resolved the CLV group-path drain (2026-05-21); the test-isolation LEAK + malformed-emitter remain open.** S155 fixed the wedge but the group path couldn't terminal-mark groupable dead records, so they clogged the drain. **S157 (committed `38c4edd`) shipped group-path terminal-marking** (`settled_without_market` / `event_no_markets_stale` / `settlement_skipped_scalar`, gated on `not _is_protected_open` + 7d grace) + a paper-only `_is_synthetic_ticker` sweep; verified live (junk cleared, 0 futures wrongly marked, 0 stale >7d, fast cycles ~240s). **Still open — the root causes S157 only symptom-treats:** (1) **Test-isolation leak (NEW, highest priority):** a test-harness emission ~2026-05-08 wrote ~930 synthetic CF rows (debug tokens TEST/ALPHA/IDL in the event-date segment) into the REAL `bot/state/clv.json`. S157's `_is_synthetic_ticker` branch cleans them but is a crutch — the leaking test must be found and isolated to a tmp/fixture (same class as the `worktree tools/ symlink` memory; tests must NEVER write to `bot/state/*.json`). **Leak-hunt in progress.** (2) **Malformed-emitter (still open):** why CF emission produced 1,025 `KXHIGHNY-26APR*` 404-forever rows — does `bot/clv.py` CF emission validate ticker shape / event liveness? Extend the "Check the Data §2 pollution check" to flag malformed event tickers. **Why it matters:** if either keeps producing junk, `settlement_failed` accumulates and CF-data-quality degrades for any `counterfactual_*` analysis. **No auto-trigger;** the test-isolation leak is the active next investigation.
