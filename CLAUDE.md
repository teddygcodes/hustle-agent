...
- **Consolidator perf regression (S110 surfaced).** `tests/test_glint_status.py::test_consolidator_completes_in_under_2s` consistently fails at 2.16-2.22s vs the 2.0s budget — not a flake (3 consecutive runs). S109 ship report recorded 1640/0; failure surfaced 24h later at S110 start with zero `bot/`/`tests/` touches in between, so the regression is from natural growth of state files the consolidator reads (test uses live REPO_ROOT, not a seeded fixture). **Unblocks:** either (a) optimize hot paths in `tools/glint_status.py:1634` (`build_snapshot`) — likely candidates are large JSON loads of `decisions.jsonl`, `paper_trades.json`, `clv.json`, `universe.jsonl` that could be streamed/cached/paginated; or (b) refactor the test to seed a minimal-fixture REPO_ROOT in a tmpdir so its budget is decoupled from state growth. Brief disallows the third option (assertion bump) because it "matches degraded behavior." Operator should pick (a) vs (b) when scoping the follow-on.
